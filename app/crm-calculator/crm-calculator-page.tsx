    ...
    <div style={{ background:'#f9fafb', borderRadius:10, padding:'16px', marginBottom:16 }}>
      ...
      <ResponsiveContainer width="100%" height={hasData.length * 44 + 20}>
        <BarChart data={hasData} layout="vertical" margin={{ left:10, right:60, top:0, bottom:0 }}>
          ...
          <Legend formatter={(v:string) => v==='active'?'In use':'Idle (at risk)'} />
          <Bar dataKey="active" name="active" stackId="a" radius={[0,0,0,0]}>
            {hasData.map((e,i) => <Cell key={i} fill={e.color} />)}
          </Bar>
          <Bar dataKey="idle" name="idle" stackId="a" radius={[0,4,4,0]}>
        ...
        <div style={{ flex:1, minWidth:0 }}>
          ...
          <SectionCard id="consumer">
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8, marginBottom:6 }}>
              <div style={{ fontSize:12, fontWeight:600, color:TEAL, textAlign:'center', padding:'4px 0' }}>✅ In use</div>
              <div style={{ fontSize:12, fontWeight:600, color:AMBER, textAlign:'center', padding:'4px 0' }}>📦 In a drawer</div>
            </div>
          ...
          <div id="results" style={{ background:'#fff', borderRadius:12, boxShadow:'0 1px 4px rgba(0,0,0,0.08)', padding:'24px 28px', marginBottom:16 }}>
            ...
            {!hasAny ? (
              ...
              <>
                ...
                {/* Material cards grid — precious metals */}
                {(['gold','silver','copper'] as MatKey[]).some(id => (totals.active[id]??0)+(totals.idle[id]??0) > 0) && (
                  <>
                    <div style={{ fontSize:13, fontWeight:700, color:'#374151', marginBottom:8 }}>💰 Precious metals (reference)</div>
                    <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:10, marginBottom:16 }}>
                      {(['gold','silver','copper'] as MatKey[]).map(id => {
                        const mat = MATERIALS.find(m=>m.id===id)!;
                        return <MaterialCard key={id} mat={mat} activeG={totals.active[id]??0} idleG={totals.idle[id]??0} />;
                      })}
                    </div>
                  </>
                )}

                {/* All strategic/critical material cards */}
                <div style={{ fontSize:13, fontWeight:700, color:'#374151', marginBottom:8 }}>All critical raw materials — detail</div>
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10, marginBottom:20 }}>
                  {MATERIALS.filter(m=>m.tier!=='reference').map(mat => (
                    <MaterialCard key={mat.id} mat={mat}
                      activeG={totals.active[mat.id]??0}
                      idleG={totals.idle[mat.id]??0} />
                  ))}
                </div>
                ...
                {hasIdle && (
                  <div style={{ background:'linear-gradient(135deg,#083344,#0e7490)', borderRadius:10, padding:'18px 22px', color:'#fff' }}>
                    ...
                    <p style={{ fontSize:13, lineHeight:1.7, margin:'0 0 12px', color:'#a5f3fc' }}>
                      Bringing your unused electronics to a <strong style={{color:'#fff'}}>Recupel collection point</strong> is the most important action you can take. Even with today's low recovery rates for critical minerals, it keeps materials in the recycling system — and recovery technology is improving rapidly with Umicore's facilities in Hoboken, Antwerp.
                    </p>
                    ...
                    <a href="https://www.recupel.be/en/find-collection-point/"
                      ...
                      🗺️ Find your nearest Recupel point →
                    </a>
                  </div>
                )}
              </>
