...
function GroupedBarChart({ data, title }: { data:{name:string;active:number;idle:number;color:string;symbol:string}[]; title:string }) {
  ...
  return (
    <div style={{ background:'#f9fafb', borderRadius:10, padding:'16px', marginBottom:16 }}>
      ...
      {/* Legend: material swatches + in use vs idle */}
      <div style={{ marginTop:10, display:'flex', flexWrap:'wrap', gap:8, justifyContent:'center' }}>
        {hasData.map(e => (
          <div key={e.name} style={{ display:'flex', alignItems:'center', gap:4, fontSize:11, color:'#4b5563' }}>
            <div style={{ width:12, height:10, borderRadius:2, background:e.color }} />
            {e.name} ({e.symbol})
          </div>
        ))}
        <div style={{ width:'100%', display:'flex', gap:14, justifyContent:'center', marginTop:4 }}>
          <div style={{ display:'flex', alignItems:'center', gap:4, fontSize:11, color:'#4b5563' }}>
            <div style={{ width:14, height:10, borderRadius:2, background:'#374151' }} />In use
          </div>
          <div style={{ display:'flex', alignItems:'center', gap:4, fontSize:11, color:'#4b5563' }}>
            <div style={{ width:14, height:10, borderRadius:2, background:'#37415140', border:'1px solid #374151' }} />Idle (at risk)
          </div>
        </div>
      </div>
    </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16 }}>
        ...
        <div style={{ background:'#f9fafb', borderRadius:10, padding:'16px' }}>
            ...
            <BarChart data={chartData} margin={{ left:10, right:10, top:4, bottom:20 }}>
              ...
              <Tooltip
                formatter={(v:number, key:string) => {
                  ...
                  const isGrey = key.endsWith('_g');
                  const matId  = isGrey ? key.slice(0,-2) : key;
                  const mat    = MATERIALS.find(m=>m.id===matId);
                  return [fmt(v), mat ? `${mat.label} (${mat.symbol})` : key];
                }}
              />
              {/* Colored bars: In drawer + Recoverable */}
              {MATERIALS.map(mat => (
                <Bar key={mat.id} dataKey={mat.id} stackId="a" fill={mat.color} legendType="none" name={mat.symbol} />
              ))}
              {/* Grey bars: Lost */}
              {MATERIALS.map((mat, i) => (
                <Bar key={`${mat.id}_g`} dataKey={`${mat.id}_g`} stackId="a" fill={GREY_SHADES[i]} legendType="none" name={mat.symbol} />
              ))}
            </BarChart>
          </ResponsiveContainer>
          {/* Summary */}
          ...
          <div style={{ display:'flex', flexWrap:'wrap', gap:5, marginTop:10 }}>
            {MATERIALS.map((mat,i)=>(
              <div key={mat.id} style={{ display:'flex', alignItems:'center', gap:3, fontSize:10, color:'#4b5563' }}>
                <div style={{ width:10, height:8, borderRadius:1, background:mat.color }} />
                {mat.symbol}
              </div>
            ))}
            <div style={{ display:'flex', alignItems:'center', gap:3, fontSize:10, color:'#6b7280' }}>
              <div style={{ width:10, height:8, borderRadius:1, background:'#9ca3af' }} />
              Lost (grey)
            </div>
          </div>
        </div>
...
function PreciousMetalsChart({ totals }: { totals:{ active:Partial<Record<MatKey,number>>; idle:Partial<Record<MatKey,number>> } }) {
  const data = (['gold','silver','copper'] as MatKey[]).map(id => {
    const mat = MATERIALS.find(m=>m.id===id)!;
    return { name:mat.label, active: totals.active[id]??0, idle: totals.idle[id]??0, color:mat.color, symbol:mat.symbol };
  });
  return <GroupedBarChart data={data} title="💰 Precious metals (reference — not EU CRM, included for context)" />;
}
