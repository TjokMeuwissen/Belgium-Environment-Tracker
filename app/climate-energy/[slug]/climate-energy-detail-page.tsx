...
const FOOTPRINT_CATEGORIES = [
  ...
  { name: 'Other',     value: 3.1,  color: '#6b7280' },
];
...
const GHG_SECTORS = [
  ...
  { name: 'Other',                   value: 1.9,  color: '#6b7280' },
];
...
// ── Standard info cards ───────────────────────────────────────────────────────
function TechnicalInfoCard({ text, notes, children }: {
  text?: string; notes?: string | null; children?: React.ReactNode;
}) {
  return (
    <div className="detail-technical-card">
      ...
      {text && <p className="technical-text">{text}</p>}
      {children}
      {notes && (
      ...
      <div style={{ width: '100%', height: 320 }}>
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={data} margin={{ top: 8, right: 24, left: 0, bottom: 0 }}>
            ...
            <XAxis dataKey="year" tick={{ fontSize: 11, fill: '#6b7280' }} tickLine={false} interval={4} />
            <YAxis tick={{ fontSize: 11, fill: '#6b7280' }} tickLine={false} axisLine={false} width={45} domain={[0, 160]} />
...
function EnergySectorsChart({ data }: { data: any[] }) {
  ...
  const chartData = data.map(d => ({ ...d, color: COLORS[d.sector] ?? '#6b7280' }));
  return (
    <div className="detail-chart-block">
      ...
      <div style={{ width: '100%', height: 260 }}>
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={chartData} layout="vertical" margin={{ top: 4, right: 64, left: 80, bottom: 4 }}>
            ...
            <XAxis type="number" tick={{ fontSize: 11, fill: '#6b7280' }} tickLine={false} domain={[0, 12]} tickFormatter={v => `${v}`} />
      ...
      <div style={{ width: '100%', height: 280 }}>
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={chartData} margin={{ top: 8, right: 24, left: 0, bottom: 4 }}>
            ...
            <YAxis tick={{ fontSize: 11, fill: '#6b7280' }} tickLine={false} axisLine={false} tickFormatter={v => `${v} GW`} />
            <Tooltip contentStyle={{ background: '#fff', border: '1px solid #e5e3da', borderRadius: 8, fontSize: 12 }}
            ...
            <Bar dataKey="Potential"        fill="#9ca3af"    radius={[3,3,0,0]} />
          </BarChart>
        </ResponsiveContainer>
      </div>
      ...
      <div style={{ width: '100%', height: 280 }}>
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={PERCAPITA_BARS} layout="vertical" margin={{ top: 4, right: 70, left: 90, bottom: 4 }}>
            <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" horizontal={false} />
            <XAxis type="number" domain={[0, 16]} tick={{ fontSize: 11, fill: '#6b7280' }} tickLine={false} tickFormatter={v => `${v} t`} />
...
// ── Explainer cards ───────────────────────────────────────────────────────────
// ── Explainer content — bold title + paragraph style inside TechnicalInfoCard ─
const SH: React.CSSProperties = { fontWeight: 700, fontSize: '0.95rem', color: '#1a1a1a', marginBottom: 6, marginTop: 18 };

function WhatIsGHGCard({ chart }: { chart?: React.ReactNode }) {
  const gases = [
  ...
  return (
    <TechnicalInfoCard>
      <p style={SH}>What is a greenhouse gas?</p>
      ...
      <p style={SH}>Global Warming Potential (GWP100)</p>
      <p className="technical-text">
        Not all GHGs are equally potent. Scientists compare them using the Global Warming Potential over 100 years (GWP100) — the total heat trapped by 1 tonne of a gas over a century, relative to CO₂. While CO₂ is the most abundant GHG, some F-gases trap tens of thousands of times more heat per molecule and persist in the atmosphere for thousands of years. The chart below plots each gas — position shows warming power, bubble size shows its share of Belgian emissions.
      </p>
      ...
      {chart && <div style={{ marginTop: 20 }}>{chart}</div>}
    </TechnicalInfoCard>
  );
}

function WhatIsFootprintCard({ chart }: { chart?: React.ReactNode }) {
  return (
    <TechnicalInfoCard>
      <p style={SH}>What is a carbon footprint?</p>
      <p className="technical-text">
        A carbon footprint measures the total GHG emissions caused by an individual, organisation or country — expressed in tonnes of CO₂ equivalent. For countries, two fundamentally different accounting methods give very different results, which explains the two figures for Belgium.
      </p>
      <p style={SH}>Territorial accounting: 8.3 t/cap (2023)</p>
      <p className="technical-text">
        Counts all emissions physically occurring on Belgian soil — from factories, cars, farms and buildings in Belgium. This is the official figure reported to the EU and UNFCCC, and what this indicator tracks.
      </p>
      <p style={SH}>Consumption-based accounting: ~15.7 t/cap (2019)</p>
      <p className="technical-text">
        Counts all emissions caused by what Belgians consume, regardless of where production happened. When Belgium imports steel from Asia or electronics from Taiwan, those manufacturing emissions are included here. This figure comes from UCLouvain research (2021) and uses a different reference year — the two are not directly comparable.
      </p>
      <p style={SH}>The ~7.4 tonne gap</p>
      <p className="technical-text">
        This gap represents Belgium&#39;s imported carbon — the CO₂ embedded in goods manufactured abroad but consumed in Belgium. It matters for policy: reducing territorial emissions by offshoring production does not reduce Belgium&#39;s true climate impact. Consumption-based accounting holds countries responsible for what they buy, not just what they produce domestically. The country comparison chart below shows where Belgium sits relative to its neighbours and the Paris 2030 fair-share target.
      </p>
      {chart && <div style={{ marginTop: 20 }}>{chart}</div>}
    </TechnicalInfoCard>
  );
...
function ElectricalVsThermalCard({ chart }: { chart?: React.ReactNode }) {
  return (
    <TechnicalInfoCard>
      {chart && <div style={{ marginBottom: 20 }}>{chart}</div>}
      <p style={SH}>Electrical energy vs thermal energy</p>
      <p className="technical-text">
        Final Energy Consumption (FEC) measures the energy actually delivered to end users — after all conversion and transmission losses. It includes two fundamentally different forms that play very different roles in the energy transition.
      </p>
      <p style={SH}>Thermal energy (~75% of Belgian FEC)</p>
      <p className="technical-text">
      ...
      <p style={SH}>Electrical energy (~25% of Belgian FEC)</p>
      <p className="technical-text">
        The versatile, zero-emission-at-point-of-use form. Electricity can power anything from a heat pump to a steel arc furnace, and can be generated from renewables. Electrification of heat and transport is central to Belgium&#39;s decarbonisation strategy.
      </p>
      <p style={SH}>Why this matters for the energy transition</p>
      <p className="technical-text">
        Electrification is more efficient than burning fuel directly. A heat pump delivering 1 unit of heat uses only ~0.33 units of electricity (coefficient of performance ≈ 3), compared to burning 1 unit of gas for 0.85–0.90 units of heat. This means total FEC can fall even as electricity demand rises. Belgium&#39;s NECP projects a roughly 40% increase in electricity demand by 2030 alongside an overall FEC reduction target — replacing gas boilers and combustion engines while keeping the grid balanced.
      </p>
    </TechnicalInfoCard>
  );
}

function GridCongestionCard({ capacityData, chart }: { capacityData: any[]; chart?: React.ReactNode }) {
  const solar    = capacityData.find(d => d.source === 'Solar PV');
  const offshore = capacityData.find(d => d.source === 'Offshore wind');
  return (
    <TechnicalInfoCard>
      {chart && <div style={{ marginBottom: 20 }}>{chart}</div>}
      <p style={SH}>Grid congestion — the infrastructure bottleneck</p>
      <p className="technical-text">
      ...
      <p style={SH}>Curtailment and negative electricity prices</p>
      <p className="technical-text">
        When more electricity is generated than the grid can transmit or consumers can absorb, grid operators must instruct generators to reduce output — wasting clean energy that has already been built and paid for.{solar ? <> Solar PV curtailment is already occurring in Belgium&#39;s distribution grid during high-generation periods. Elia recorded negative wholesale electricity prices <strong>6.1% of the time in 2025</strong> — meaning generators were effectively paying to export power.</> : null}
      </p>
      <p style={SH}>Belgium&#39;s infrastructure bottlenecks</p>
      <p className="technical-text">
        <strong>Ventilus</strong> (West Flanders): a new 380 kV high-voltage line critical for connecting offshore wind to the national grid — its permit process faced years of delays due to local opposition and environmental procedures. <strong>Boucle du Hainaut</strong> (Wallonia): a grid reinforcement ring essential for southern Belgium&#39;s electricity capacity — also behind schedule.{offshore ? <> Together, these delays constrain how much additional offshore capacity can be delivered: the Princess Elisabeth Zone alone has seen farms delayed by 3+ years per Elia&#39;s 2025 assessment.</> : null}
      </p>
      <p style={SH}>Solutions</p>
      <p className="technical-text">
  ...
  if (slug === 'total-ghg-emissions') {
    ...
    chartNode = (
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24 }}>
        ...
        <PieChartBlock data={GHG_SECTORS} title={"Territorial GHG emissions by sector (Belgium, 2023)"} source="Source: National Climate Commission (2025)" />
      </div>
    );
        ...
        <div className="detail-main">
          ...
          <div className="detail-info">
            ...
            <div id="technical-info">
              {/* total-ghg-emissions — bubble chart inside the card, after the table */}
              {slug === 'total-ghg-emissions' && (
                <WhatIsGHGCard chart={<GHGBubbleChart />} />
              )}

              {/* per-capita-ghg-footprint — comparison chart inside the card, after the text */}
              {slug === 'per-capita-ghg-footprint' && (
                <WhatIsFootprintCard chart={<PerCapitaComparisonChart />} />
              )}

              {/* final-energy-consumption — sector chart inside the card, before the text */}
              {slug === 'final-energy-consumption' && (
                <ElectricalVsThermalCard chart={finalEnergyData.length > 0 ? <EnergySectorsChart data={finalEnergyData} /> : undefined} />
              )}

              {/* renewable-electricity — capacity chart inside the card, before the text */}
              {slug === 'renewable-electricity-share-of-generation' && (
                <GridCongestionCard capacityData={renewableCapData} chart={renewableCapData.length > 0 ? <RenewableCapacityChart data={renewableCapData} /> : undefined} />
              )}
