    ...
    <div className="detail-technical-card">
      ...
      <div style={{ marginBottom: 24 }}>
        <p style={{ fontSize: '0.88rem', lineHeight: 1.75, color: '#374151', marginBottom: 10 }}>
          <strong style={{ display: 'block', marginBottom: 4 }}>What is a greenhouse gas?</strong>Greenhouse gases (GHGs) are atmospheric gases that
        ...
        <p style={{ fontSize: '0.88rem', lineHeight: 1.75, color: '#374151', marginBottom: 10 }}>
          <strong style={{ display: 'block', marginBottom: 4 }}>What is GWP?</strong>Global Warming Potential (GWP) measures how much heat a gas
        ...
        <p style={{ fontSize: '0.88rem', lineHeight: 1.75, color: '#374151', margin: 0 }}>
          <strong style={{ display: 'block', marginBottom: 4 }}>Why does it lead to climate change?</strong>As GHG concentrations increase, more
  ...
  return (
    <div className="detail-technical-card">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 12 }}>
        <div style={{ background: '#f0f9ff', borderRadius: 8, padding: '14px 16px', borderLeft: '4px solid #3b82f6' }}>
...
// ── Per-capita: historical trend + country comparison ─────────────────────────

function PerCapitaTechnicalCard({
  historicalPerCapita,
  countryComparison,
  footprintExamples,
}: {
  historicalPerCapita: { year: number; value: number }[];
  countryComparison: any[];
  footprintExamples: any[];
}) {
  return (
    <div className="detail-technical-card">
      <div className="detail-section-header">
      ...
      <div style={{ marginBottom: 22 }}>
        <p style={{ fontSize: '0.88rem', lineHeight: 1.75, color: '#374151', marginBottom: 10 }}>
          <strong style={{ display: 'block', marginBottom: 4 }}>What is a carbon footprint?</strong>A carbon footprint is the total amount of
          greenhouse gas emissions caused — directly or indirectly — by a person, organisation,
          product or activity. It is expressed in tonnes of CO₂-equivalent (tCO₂eq), which converts
          all GHGs into a single comparable unit using their Global Warming Potential (GWP).
        </p>
        <p style={{ fontSize: '0.88rem', lineHeight: 1.75, color: '#374151', marginBottom: 10 }}>
          Belgium's <strong>territorial footprint</strong> (8.3 t/cap) only counts emissions
          released on Belgian soil. The <strong>consumption-based footprint</strong> (~15.7 t/cap)
          adds emissions from imported goods produced abroad. The gap reflects Belgium's position
          as a net importer of carbon-intensive products.
        </p>
      </div>

      {/* ── Real-life examples ── */}
      <div style={{ borderTop: '1px solid #e5e7eb', paddingTop: 18, marginBottom: 22 }}>
        <div style={{ fontWeight: 700, fontSize: '0.9rem', color: '#1a1a1a', marginBottom: 4 }}>
          Putting 8.3 tonnes into perspective
        </div>
        <p style={{ fontSize: '0.82rem', color: '#6b7280', marginBottom: 12, lineHeight: 1.5 }}>
          The average Belgian emits 8.3 tCO₂eq per year. Here's what that looks like in everyday activities:
        </p>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10 }}>
          {footprintExamples.slice(0, 6).map((ex: any, i: number) => {
            const icons = ['🚗','⚡','🥩','🏠','✈️','🍽️'];
            const isLarge = ex.kg_co2eq >= 1000;
            const display = isLarge
              ? `${(ex.kg_co2eq / 1000).toFixed(1)} tCO₂eq`
              : `${ex.kg_co2eq} kg`;
            return (
              <div key={i} style={{ background: '#f8fafc', borderRadius: 8, padding: '12px 14px', border: '1px solid #e5e7eb', textAlign: 'center' }}>
                <div style={{ fontSize: '1.6rem', marginBottom: 4 }}>{icons[i]}</div>
                <div style={{ fontSize: '0.72rem', color: '#6b7280', lineHeight: 1.3, marginBottom: 6 }}>
                  {String(ex.activity).replace(' — average petrol car', '').replace(' — average EV (Belgian grid 2023)', ' (EV)').replace('Heating a 100m² house for one winter day (natural gas)', 'House heating (1 winter day, gas)')}
                </div>
                <div style={{ fontSize: '1rem', fontWeight: 700, color: '#1a1a1a' }}>{display}</div>
              </div>
            );
          })}
        </div>
        <p style={{ fontSize: '0.72rem', color: '#9ca3af', marginTop: 8, lineHeight: 1.5 }}>
          Sources: {footprintExamples[0]?.source ?? 'EEA / ADEME'}. Values are indicative averages — individual results vary.
        </p>
      </div>
      ...
      <div style={{ borderTop: '1px solid #e5e7eb', paddingTop: 18, marginBottom: 22 }}>
        <div style={{ fontWeight: 700, fontSize: '0.9rem', color: '#1a1a1a', marginBottom: 4 }}>
          Historical per-capita trend — Belgium 1990–2023
        </div>
        <p style={{ fontSize: '0.82rem', color: '#6b7280', marginBottom: 10, lineHeight: 1.5 }}>
          Derived from national GHG inventory totals divided by Belgian population (Statbel).
          Despite a 42% reduction since 1990, Belgium remains far above the 2030 Paris fair-share target.
        </p>
        <ResponsiveContainer width="100%" height={240}>
          <LineChart data={historicalPerCapita} margin={{ top: 8, right: 24, left: 0, bottom: 0 }}>
            <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" />
            <XAxis dataKey="year" tick={{ fontSize: 11, fill: '#9ca3af' }} tickLine={false} interval={4} />
            <YAxis tick={{ fontSize: 11, fill: '#9ca3af' }} tickLine={false} axisLine={false} width={36} domain={[0, 16]} tickFormatter={v => `${v}t`} />
            <Tooltip
              contentStyle={{ background: '#fff', border: '1px solid #e5e3da', borderRadius: 8, fontSize: 12 }}
              formatter={(v: any) => [`${v} tCO₂eq/cap`, 'Per-capita emissions']}
              labelStyle={{ fontWeight: 700 }}
            />
            <Line type="monotone" dataKey="value" stroke="#f97316" strokeWidth={2.5} dot={false} activeDot={{ r: 4 }} />
            <ReferenceLine y={2.3} stroke="#16a34a" strokeDasharray="5 4" strokeWidth={1.8}
              label={{ value: '🎯 2030 Paris target: 2.3 t', position: 'insideTopRight', fontSize: 10, fill: '#16a34a', fontWeight: 600 }} />
            <ReferenceLine y={8.3} stroke="#9ca3af" strokeDasharray="3 3" strokeWidth={1}
              label={{ value: 'Current: 8.3 t (2023)', position: 'insideBottomRight', fontSize: 10, fill: '#9ca3af' }} />
          </LineChart>
        </ResponsiveContainer>
        <p style={{ fontSize: '0.73rem', color: '#9ca3af', marginTop: 6 }}>
          Source: National Climate Commission / indicators.be — population: Statbel
        </p>
      </div>
      ...
      <div style={{ borderTop: '1px solid #e5e7eb', paddingTop: 18 }}>
        <div style={{ fontWeight: 700, fontSize: '0.9rem', color: '#1a1a1a', marginBottom: 4 }}>
          Belgium vs neighbouring countries — 2023 (tCO₂eq/cap, territorial)
        </div>
        <p style={{ fontSize: '0.82rem', color: '#6b7280', marginBottom: 10, lineHeight: 1.5 }}>
          Belgium sits above both the EU average and France, but below Germany and the Netherlands.
          Luxembourg's figure is inflated by fuel tourism.
        </p>
        <ResponsiveContainer width="100%" height={220}>
          <BarChart data={countryComparison} layout="vertical" margin={{ top: 4, right: 60, left: 120, bottom: 4 }}>
            <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" horizontal={false} />
            <XAxis type="number" domain={[0, 15]} tick={{ fontSize: 11, fill: '#9ca3af' }} tickLine={false} tickFormatter={v => `${v}t`} />
            <YAxis type="category" dataKey="country" tick={{ fontSize: 11, fill: '#4b5563' }} tickLine={false} axisLine={false} width={120} />
            <Tooltip
              contentStyle={{ background: '#fff', border: '1px solid #e5e3da', borderRadius: 8, fontSize: 12 }}
              formatter={(v: any, _: any, props: any) => [
                `${v} tCO₂eq/cap — ${props.payload.notes?.split('.')[0] ?? ''}`,
                props.payload.country,
              ]}
            />
            <Bar dataKey="value" radius={[0, 4, 4, 0]}>
              {countryComparison.map((c: any, i: number) => (
                <Cell key={i} fill={
                  c.country === 'Belgium' ? '#f97316' :
                  c.country === 'Paris 2030 target' ? '#16a34a' :
                  c.country === 'EU average' ? '#6366f1' : '#94a3b8'
                } />
              ))}
            </Bar>
          </BarChart>
        ...
        <p style={{ fontSize: '0.73rem', color: '#9ca3af', marginTop: 6 }}>
          Source: EEA national inventories / EDGAR v8 (2023). All values territorial incl. LULUCF. Luxembourg inflated by fuel tourism.
        </p>
      </div>
    </div>
  );
...
// ── Final Energy: technical description + sector trend ────────────────────────


// ── Inline Mtoe converter (used inside FinalEnergyTechnicalCard) ──────────────
function MtoeConverterInline() {
  const [value, setValue] = useState(30);
  const twh     = +(value * 11.63).toFixed(1);
  const carBkm  = +(value * 18.2).toFixed(1);
  const homesM  = +(value * 5.3).toFixed(1);
  ...
  return (
    <div style={{ borderTop: '1px solid #e5e7eb', paddingTop: 18 }}>
      <div style={{ fontWeight: 700, fontSize: '0.9rem', color: '#1a1a1a', marginBottom: 4 }}>
        Unit converter — what does 1 Mtoe mean?
      </div>
      <p style={{ fontSize: '0.82rem', color: '#6b7280', marginBottom: 12, lineHeight: 1.5 }}>
        Drag the slider to explore real-world equivalents for any amount of Mtoe.
      </p>
      <input type="range" min={1} max={50} value={value}
        onChange={e => setValue(Number(e.target.value))} style={{ width: '100%', marginBottom: 8 }} />
      <p style={{ textAlign: 'center', fontSize: '1.4rem', fontWeight: 700, color: '#1a1a1a', margin: '4px 0 12px' }}>
        {value} Mtoe
      </p>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 8 }}>
        {[
          { icon: '⚡', label: 'Electricity', val: `${twh} TWh` },
          { icon: '🚗', label: 'Bn km by car', val: `${carBkm}B km` },
          { icon: '🏠', label: 'Homes heated (1yr)', val: `${homesM}M` },
          { icon: '✈️', label: 'Flights BRU–NYC', val: `${flightM}M` },
        ].map(item => (
          <div key={item.label} style={{ background: '#f8fafc', borderRadius: 8, padding: '10px', textAlign: 'center', border: '1px solid #e5e7eb' }}>
            <div style={{ fontSize: '1.3rem', marginBottom: 2 }}>{item.icon}</div>
            <div style={{ fontSize: '0.67rem', color: '#6b7280', textTransform: 'uppercase', letterSpacing: '0.04em', marginBottom: 3, lineHeight: 1.2 }}>{item.label}</div>
            <div style={{ fontSize: '0.88rem', fontWeight: 700, color: '#1a1a1a' }}>{item.val}</div>
          </div>
        ))}
      </div>
      <p style={{ fontSize: '0.72rem', color: '#9ca3af', marginTop: 8, lineHeight: 1.4 }}>
        Approx. equivalents: car at 7L/100km petrol; homes gas heating ~2.2 MWh/yr; return BRU–NYC economy.
      </p>
    </div>
  );
}

function FinalEnergyTechnicalCard({
  fecHistorical,
  fecSectors,
}: {
  fecHistorical: any[];
  ...
  const SECTOR_COLORS: Record<string, string> = {
    Industry: '#f97316', Transport: '#3b82f6', Households: '#22c55e',
    Services: '#8b5cf6', Agriculture: '#06b6d4',
  };

  const chartData = [
    { year: '2005' }, { year: '2010' }, { year: '2015' }, { year: '2020' }, { year: '2022' },
  ].map(d => {
    const row: any = { year: d.year };
    fecHistorical.forEach((s: any) => {
      const key = `y${d.year}` as keyof typeof s;
      if (s[key] != null) row[s.sector] = s[key];
    });
    return row;
  });

  const sectors = fecHistorical.map((s: any) => s.sector);
  ...
  return (
    <div className="detail-technical-card">
      ...
      {/* ── What final energy consumption means ── */}
      <div style={{ marginBottom: 22 }}>
        <p style={{ fontSize: '0.88rem', lineHeight: 1.75, color: '#374151', marginBottom: 10 }}>
          <strong style={{ display: 'block', marginBottom: 4 }}>What does final energy consumption represent?</strong>Final energy consumption
          (FEC) is the total energy delivered to end users — industry, transport, households and
          services — after losses in conversion and distribution. It excludes energy used by the
          energy sector itself (e.g. refineries, power plants).
        </p>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 10 }}>
          {[
            { label: 'Electricity', icon: '⚡', text: 'Powers appliances, lighting, motors, EVs. Generated from nuclear, gas, wind, solar. ~18% of Belgian FEC.' },
            { label: 'Natural gas', icon: '🔥', text: 'Dominates space heating in homes and industry. Imported mainly from Norway, Algeria and via LNG terminals. ~23% of FEC.' },
            { label: 'Oil products', icon: '⛽', text: 'Petrol, diesel, kerosene. Almost entirely for road transport and aviation. Imported via the Antwerp refinery cluster. ~38% of FEC.' },
            { label: 'Renewables', icon: '🌿', text: 'Biomass for heat, wind/solar for electricity, heat pumps. Growing share. ~13% of FEC — mostly biomass, not yet electrification.' },
          ].map((item, i) => (
            <div key={i} style={{ background: '#f8fafc', borderRadius: 8, padding: '12px 14px', border: '1px solid #e5e7eb' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                <span style={{ fontSize: '1.2rem' }}>{item.icon}</span>
                <span style={{ fontWeight: 700, fontSize: '0.85rem', color: '#1a1a1a' }}>{item.label}</span>
              </div>
              <p style={{ fontSize: '0.8rem', color: '#4b5563', margin: 0, lineHeight: 1.55 }}>{item.text}</p>
            </div>
          ))}
        </div>
        <div style={{ background: '#f0f9ff', borderRadius: 8, padding: '12px 14px', borderLeft: '4px solid #3b82f6' }}>
          <div style={{ fontWeight: 700, fontSize: '0.83rem', color: '#1e3a5f', marginBottom: 4 }}>Scale reference</div>
          <p style={{ fontSize: '0.81rem', color: '#374151', margin: 0, lineHeight: 1.6 }}>
            Belgium's total FEC of 30 Mtoe is equivalent to the annual output of roughly
            <strong> 125 large gas power plants</strong> (400 MW each at 80% load) or
            <strong> ~12,500 modern offshore wind turbines</strong> (8 MW, 40% capacity factor).
            A single large gas plant produces ~0.24 Mtoe/year; a single offshore turbine ~0.0024 Mtoe/year.
          </p>
        </div>
      </div>
      ...
      <div style={{ borderTop: '1px solid #e5e7eb', paddingTop: 18 }}>
        <div style={{ fontWeight: 700, fontSize: '0.9rem', color: '#1a1a1a', marginBottom: 4 }}>
          Final energy consumption by sector — 2005 to 2022 (Mtoe)
        </div>
        <p style={{ fontSize: '0.82rem', color: '#6b7280', marginBottom: 12, lineHeight: 1.5 }}>
          Industry and households have reduced their energy use significantly since 2005. Transport barely changed until the COVID dip in 2020, with a partial rebound in 2022.
        </p>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, marginBottom: 8 }}>
          {sectors.map((s: string) => (
            <div key={s} style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 12, color: '#4b5563' }}>
              <div style={{ width: 10, height: 10, borderRadius: 2, background: SECTOR_COLORS[s] ?? '#94a3b8' }} />
              <span>{s}</span>
            </div>
          ))}
        </div>
        <ResponsiveContainer width="100%" height={260}>
          <BarChart data={chartData} margin={{ top: 4, right: 16, left: 0, bottom: 4 }}>
            ...
            <YAxis tick={{ fontSize: 11, fill: '#9ca3af' }} tickLine={false} axisLine={false} width={40} tickFormatter={v => `${v}`} unit=" Mt" />
            <Tooltip
              contentStyle={{ background: '#fff', border: '1px solid #e5e3da', borderRadius: 8, fontSize: 12 }}
              formatter={(v: any, name: string) => [`${v} Mtoe`, name]}
            />
            {sectors.map((s: string) => (
              <Bar key={s} dataKey={s} stackId="a" fill={SECTOR_COLORS[s] ?? '#94a3b8'} />
            ))}
          </BarChart>
        </ResponsiveContainer>
        <p style={{ fontSize: '0.73rem', color: '#9ca3af', marginTop: 6 }}>
          Source: Eurostat (nrg_bal_c), Belgium 2005–2022
        </p>
      </div>

      {/* ── Sector breakdown (current year) ── */}
      {fecSectors.length > 0 && (
        <div style={{ borderTop: '1px solid #e5e7eb', paddingTop: 18 }}>
          <div style={{ fontWeight: 700, fontSize: '0.9rem', color: '#1a1a1a', marginBottom: 4 }}>
            Final energy by sector — 2022 (%)
          </div>
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={fecSectors.filter((s: any) => s.sector !== 'TOTAL')} layout="vertical" margin={{ top: 4, right: 60, left: 100, bottom: 4 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" horizontal={false} />
              <XAxis type="number" tick={{ fontSize: 11, fill: '#9ca3af' }} tickLine={false} unit="%" domain={[0, 40]} />
              <YAxis type="category" dataKey="sector" tick={{ fontSize: 11, fill: '#4b5563' }} tickLine={false} axisLine={false} width={100} />
              <Tooltip
                contentStyle={{ background: '#ffffff', border: '1px solid #e5e3da', borderRadius: 8, fontSize: 13 }}
                formatter={(v: any, _: any, props: any) => [`${v}% (${props.payload.mtoe} Mtoe)`, 'Share of FEC']}
              />
              <Bar dataKey="pct" radius={[0, 4, 4, 0]}>
                {fecSectors.filter((s: any) => s.sector !== 'TOTAL').map((_: any, i: number) => (
                  <Cell key={i} fill={['#f97316','#3b82f6','#22c55e','#8b5cf6','#06b6d4'][i % 5]} />
                ))}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
          <p style={{ fontSize: '0.73rem', color: '#9ca3af', marginTop: 4 }}>Source: Eurostat (nrg_bal_c), Belgium 2022.</p>
        </div>
      )}
...
// ── Renewable: technical description + capacity vs potential ──────────────────

function RenewableTechnicalCard({ renewableCapacity }: { renewableCapacity: any[] }) {
  const offshore = renewableCapacity.find((r: any) => r.source === 'Offshore wind');
  const onshore  = renewableCapacity.find((r: any) => r.source === 'Onshore wind');
  const solar    = renewableCapacity.find((r: any) => r.source === 'Solar PV');

  const CapacityBar = ({ data }: { data: any }) => {
    if (!data) return null;
    const pct = Math.round((data.installed_gw / data.potential_gw) * 100);
    const pct2030 = Math.round((data.target_2030_gw / data.potential_gw) * 100);
    return (
      <div style={{ marginBottom: 16 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 4 }}>
          <span style={{ fontWeight: 700, fontSize: '0.88rem', color: '#1a1a1a' }}>{data.source}</span>
          <span style={{ fontSize: '0.78rem', color: '#6b7280' }}>
            {data.installed_gw} GW installed / {data.target_2030_gw} GW target / {data.potential_gw} GW potential
          </span>
        </div>
        <div style={{ position: 'relative', height: 20, background: '#f3f4f6', borderRadius: 10, overflow: 'hidden' }}>
          <div style={{ position: 'absolute', left: 0, top: 0, bottom: 0, width: `${pct2030}%`, background: '#bbf7d0', borderRadius: 10 }} />
          <div style={{ position: 'absolute', left: 0, top: 0, bottom: 0, width: `${pct}%`, background: '#16a34a', borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'flex-end', paddingRight: 6 }}>
            <span style={{ fontSize: '0.68rem', color: '#fff', fontWeight: 700 }}>{pct}%</span>
          </div>
        </div>
        <div style={{ display: 'flex', gap: 12, marginTop: 4, fontSize: '0.72rem', color: '#6b7280' }}>
          <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
            ...
            Installed ({data.installed_gw} GW)
          </span>
          <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
            <span style={{ width: 8, height: 8, borderRadius: 2, background: '#bbf7d0', display: 'inline-block' }} />
            2030 target ({data.target_2030_gw} GW)
          </span>
          <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
            <span style={{ width: 8, height: 8, borderRadius: 2, background: '#e5e7eb', display: 'inline-block' }} />
            Long-term potential ({data.potential_gw} GW by {data.potential_year})
          </span>
        </div>
        {data.barriers && (
          <div style={{ marginTop: 8, padding: '8px 12px', background: '#fff7ed', borderRadius: 6, borderLeft: '3px solid #f97316' }}>
            <div style={{ fontSize: '0.7rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em', color: '#92400e', marginBottom: 3 }}>Main barriers</div>
            <p style={{ fontSize: '0.79rem', color: '#4b5563', margin: 0, lineHeight: 1.55 }}>{data.barriers}</p>
          </div>
        )}
      </div>
    );
    ...
    <div className="detail-technical-card">
      ...
      <div style={{ marginBottom: 22 }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 14 }}>
          <div style={{ background: '#f0f9ff', borderRadius: 8, padding: '14px 16px', borderLeft: '4px solid #0369a1' }}>
            <div style={{ fontWeight: 700, fontSize: '0.88rem', color: '#1e3a5f', marginBottom: 6 }}>💨 Wind energy</div>
            <p style={{ fontSize: '0.81rem', color: '#374151', margin: 0, lineHeight: 1.65 }}>
              Wind turbines convert kinetic energy from moving air into electricity via a rotor and generator.
              Offshore turbines (like Belgium's North Sea fleet) benefit from stronger, more consistent
              winds than onshore — achieving capacity factors of ~40% vs ~25% on land.
              Belgium has 9 offshore wind farms with a combined capacity of 2.26 GW, producing
              ~8 TWh/year, enough for roughly 15% of Belgium's electricity demand.
              Main offshore parks: C-Power (Thorntonbank), Belwind, Northwind, Nobelwind, Rentel, Norther, SeaMade.
            </p>
          </div>
          <div style={{ background: '#fefce8', borderRadius: 8, padding: '14px 16px', borderLeft: '4px solid #ca8a04' }}>
            <div style={{ fontWeight: 700, fontSize: '0.88rem', color: '#713f12', marginBottom: 6 }}>☀️ Solar PV</div>
            <p style={{ fontSize: '0.81rem', color: '#374151', margin: 0, lineHeight: 1.65 }}>
              Photovoltaic panels convert sunlight directly into electricity using semiconductor
              materials. Belgium has ~12 GW of installed solar capacity — mainly rooftop residential
              panels — producing ~13% of electricity consumption in 2024 despite relatively low
              irradiance compared to southern Europe.
              Growth has been exceptional (+1.4 GW added in 2024 alone), but the distribution
              grid is increasingly struggling to absorb peak solar generation.
            </p>
          </div>
        </div>

        {/* Grid congestion box */}
        <div style={{ background: '#fef2f2', borderRadius: 8, padding: '14px 16px', borderLeft: '4px solid #dc2626' }}>
          <div style={{ fontWeight: 700, fontSize: '0.85rem', color: '#7f1d1d', marginBottom: 6 }}>
            ⚠️ Grid congestion — the main challenge for further expansion
          </div>
          <p style={{ fontSize: '0.81rem', color: '#374151', margin: '0 0 8px', lineHeight: 1.65 }}>
            As renewable capacity grows, the grid must handle periods of very high generation
            (sunny/windy days) that can exceed local demand. Belgium experienced negative
            electricity prices 6.1% of the time in 2025 — meaning generators were paying
            to offload surplus power.
          </p>
          <p style={{ fontSize: '0.81rem', color: '#374151', margin: 0, lineHeight: 1.65 }}>
            For offshore wind, the <strong>Ventilus</strong> high-voltage line through West-Flanders
            (connecting the North Sea to the inland grid) has faced permit delays,
            and Elia's <strong>Boucle du Hainaut</strong> grid reinforcement is behind schedule.
            This is directly delaying the Princess Elisabeth Zone offshore expansion.
            Solutions being developed: large-scale battery storage, smart grids that match
            consumption to renewable availability, demand response, and interconnections with
      ...
      <div style={{ borderTop: '1px solid #e5e7eb', paddingTop: 18 }}>
        <div style={{ fontWeight: 700, fontSize: '0.9rem', color: '#1a1a1a', marginBottom: 4 }}>
          Current capacity vs. 2030 target vs. long-term potential
        </div>
        <p style={{ fontSize: '0.82rem', color: '#6b7280', marginBottom: 14, lineHeight: 1.5 }}>
          Progress bars show installed capacity as a share of estimated long-term technical potential.
          The light green section shows the 2030 target.
        </p>
        <CapacityBar data={offshore} />
        <CapacityBar data={onshore} />
        <CapacityBar data={solar} />
        <p style={{ fontSize: '0.73rem', color: '#9ca3af', marginTop: 4 }}>
          Sources: Elia (2025); BOP; IEA-PVPS Belgium 2024; FPS Economy Belgium. Capacity as of end 2024.
        </p>
      </div>
      ...
      <div style={{ borderTop: '1px solid #e5e7eb', paddingTop: 18 }}>
        <div style={{ fontWeight: 700, fontSize: '0.9rem', color: '#1a1a1a', marginBottom: 4 }}>
          Lifecycle CO₂ intensity per renewable source (g CO₂-eq./kWh)
        </div>
        <p style={{ fontSize: '0.82rem', color: '#6b7280', marginBottom: 12, lineHeight: 1.5 }}>
          Full lifecycle including manufacturing, installation, operation and decommissioning. Global ranges from IPCC AR6.
        </p>
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.84rem' }}>
            <thead>
              <tr style={{ borderBottom: '2px solid #e5e7eb', background: '#f9fafb' }}>
                <th style={{ textAlign: 'left', padding: '8px 12px', fontWeight: 700, color: '#374151' }}>Source</th>
                <th style={{ textAlign: 'center', padding: '8px 12px', fontWeight: 700, color: '#374151' }}>Range</th>
                <th style={{ textAlign: 'center', padding: '8px 12px', fontWeight: 700, color: '#374151' }}>Median</th>
                <th style={{ textAlign: 'left', padding: '8px 12px', fontWeight: 700, color: '#374151' }}>Key factors</th>
              </tr>
            </thead>
            <tbody>
              {CO2_INTENSITY.map((row, i) => (
                <tr key={i} style={{ borderBottom: '1px solid #f3f4f6', background: i % 2 === 0 ? '#ffffff' : '#fafafa' }}>
                  ...
                  <td style={{ padding: '10px 12px', textAlign: 'center', fontFamily: 'monospace' }}>{row.range}</td>
                  <td style={{ padding: '10px 12px', textAlign: 'center', fontWeight: 700, color: '#16a34a' }}>{row.median}</td>
                  <td style={{ padding: '10px 12px', color: '#6b7280', fontSize: '0.79rem', lineHeight: 1.4 }}>{row.keyFactors}</td>
                </tr>
              ))}
        ...
        <div style={{ marginTop: 12, display: 'flex', flexDirection: 'column', gap: 6 }}>
          {CO2_INTENSITY.map((row, i) => (
            <div key={i} style={{ fontSize: '0.79rem', color: '#4b5563', display: 'flex', gap: 8, lineHeight: 1.5 }}>
              <span style={{ fontWeight: 600, flexShrink: 0 }}>{row.source}:</span>
              <span>{row.belgiumNote}</span>
            </div>
          ))}
...
// ── Section definitions for sidebar TOC ──────────────────────────────────────
...
function DetailSidebar({ slug, topicColor }: { slug: string; topicColor: string }) {
...
export default function ClimateDetailPage() {
  ...
  const fecSectors: any[]        = data.climate_supplementary?.final_energy_by_sector ?? [];
  const fecHistorical: any[]     = data.climate_supplementary?.fec_historical_by_sector ?? [];
  const countryComparison: any[] = data.climate_supplementary?.country_comparison ?? [];
  const footprintExamples: any[] = data.climate_supplementary?.footprint_examples ?? [];
  const renewableCapacity: any[] = data.climate_supplementary?.renewable_capacity ?? [];
  const allLimitations: any[]    = data.climate_supplementary?.limitations ?? [];
  const indicatorLimitations     = allLimitations.filter((l: any) => l.indicator === indicatorName);

  // Derive historical per-capita from total GHG + approximate population series
  const POP: Record<number, number> = {
    1990:10.0,1991:10.0,1992:10.1,1993:10.1,1994:10.1,1995:10.1,1996:10.2,1997:10.2,
    1998:10.2,1999:10.2,2000:10.2,2001:10.3,2002:10.3,2003:10.4,2004:10.4,2005:10.5,
    2006:10.5,2007:10.6,2008:10.7,2009:10.8,2010:10.8,2011:11.0,2012:11.1,2013:11.1,
    2014:11.2,2015:11.2,2016:11.3,2017:11.4,2018:11.4,2019:11.5,2020:11.5,2021:11.6,
    2022:11.6,2023:11.7,
  };
  const historicalPerCapita = historicalGHG
    .filter((d: any) => POP[d.year])
    .map((d: any) => ({ year: d.year, value: +(d.value / POP[d.year]).toFixed(2) }));
  ...
  if (slug === 'total-ghg-emissions') {
    ...
    technicalNode = (
      <PerCapitaTechnicalCard
        historicalPerCapita={historicalPerCapita}
        countryComparison={countryComparison}
        footprintExamples={footprintExamples}
      />
    );
  } else if (slug === 'final-energy-consumption') {
    technicalNode = (
      <FinalEnergyTechnicalCard
        fecHistorical={fecHistorical}
        fecSectors={fecSectors}
      />
    );
  } else if (slug === 'renewable-electricity-share-of-generation') {
    technicalNode = <RenewableTechnicalCard renewableCapacity={renewableCapacity} />;
  }
  ...
  return (
    <div className="detail-page">
      ...
      <div className="detail-body">
        <DetailSidebar slug={slug} topicColor="#f97316" />
        ...
        <div className="detail-info">
          ...
          <div id="limitations"><LimitationsSection limitations={indicatorLimitations} /></div>
