...
function SUVEffectChart() {
  return (
    <ResponsiveContainer width="100%" height={340}>
      <ComposedChart data={fleetData} margin={{ top: 16, right: 60, left: 8, bottom: 4 }}>
        <CartesianGrid strokeDasharray="3 3" stroke="#e5e7eb" />
        <XAxis dataKey="year" tick={{ fontSize: 11, fill: '#374151' }} interval={3} />
        <YAxis
          yAxisId="co2"
          domain={[80, 180]}
          ticks={[80, 100, 120, 140, 160, 180]}
          tick={{ fontSize: 11, fill: '#374151' }}
          label={{ value: 'CO2 (g/km)', angle: -90, position: 'insideLeft', offset: 10, style: { fontSize: 11, fill: '#374151' } }}
        />
        <YAxis
          yAxisId="mass"
          orientation="right"
          domain={[1200, 1550]}
          ticks={[1200, 1250, 1300, 1350, 1400, 1450, 1500, 1550]}
          tick={{ fontSize: 11, fill: '#374151' }}
          label={{ value: 'Mass (kg)', angle: 90, position: 'insideRight', offset: 10, style: { fontSize: 11, fill: '#374151' } }}
        />
        <Tooltip content={<FleetTooltip />} />
        <Legend
          wrapperStyle={{ fontSize: '0.8rem', paddingTop: 10, color: '#374151' }}
          formatter={(value) => <span style={{ color: '#374151', fontSize: '0.8rem' }}>{value}</span>}
        />

        {/* Shaded reversal zone 2015–2019 */}
        <ReferenceArea yAxisId="co2" x1={2015} x2={2019} fill="#fef3c7" fillOpacity={0.7} />

        {/* Series break line at 2020 */}
        <ReferenceLine yAxisId="co2" x={2020} stroke="#6b7280" strokeDasharray="4 3"
          label={{ value: 'NEDC to WLTP', position: 'top', fontSize: 10, fill: '#6b7280' }} />

        {/* Mass bars */}
        <Bar yAxisId="mass" dataKey="mass" name="Avg mass (kg)" fill="#cbd5e1" radius={[2, 2, 0, 0]} legendType="square" />

        {/* CO2 lines */}
        <Line yAxisId="co2" type="monotone" dataKey="nedc" name="CO2 NEDC (g/km)" stroke="#f97316" strokeWidth={2.5} dot={false} connectNulls={false} legendType="line" />
        <Line yAxisId="co2" type="monotone" dataKey="wltp" name="CO2 WLTP (g/km)" stroke="#ec4899" strokeWidth={2.5} strokeDasharray="5 3" dot={false} connectNulls={false} legendType="line" />
      </ComposedChart>
    </ResponsiveContainer>
  );
}
...
export default function HowCleanIsYourCar() {
  return (
    <div className="detail-page">
      ...
      <div className="detail-header" style={{ background: 'linear-gradient(135deg, #1e3a5f 0%, #1d4ed8 60%, #3b82f6 100%)', '--topic-color': TOPIC_COLOR } as React.CSSProperties}>
      ...
      <div className="detail-body">
        ...
        <div className="detail-main">
          ...
          <SectionCard id="lifecycle">
            ...
            <Para>
              <strong>Important: the Life Cycle Impact is not a carbon footprint.</strong> It covers the
              full lifecycle across three phases &mdash; vehicle production (including the battery), the
              fuel or electricity supply chain, and driving &mdash; and it goes beyond climate change to
              also include impacts on human health (fine particles, smog) and ecosystems (acidification,
              eutrophication). This is why diesel, despite having similar official CO&#x2082; figures to
              petrol, scores dramatically worse overall: its particle and NOx emissions carry a heavy
              health penalty that simply does not appear in a CO&#x2082;-only comparison.
            </Para>

            <LCIChart />

            <BulletList items={[
              { bold: 'BEV is always the best choice within the same segment', text: 'Even the heavier, longer-range 600\u00a0km BEV (LCI\u00a0216) outperforms every combustion option, including the petrol hybrid (229) and petrol (240).' },
              { bold: 'Diesel is always the worst', text: 'A diesel small family car scores 321 \u2014 nearly twice the impact of the best BEV \u2014 due to its disproportionate air quality penalty.' },
              { bold: 'PHEVs only clean up when regularly charged', text: 'A PHEV with a 65\u00a0km electric range scores 248, worse than a plain petrol car (240), because its battery adds manufacturing emissions without enough real-world electric driving to compensate. A PHEV with a 130\u00a0km range scores 195 \u2014 but only if the battery is consistently charged.' },
            ]} />
