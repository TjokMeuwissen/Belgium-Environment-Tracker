...
const CARBON_DATA = [
  { name: 'Peat bogs',        value: 600, color: '#78350f' },
  { name: 'Wetlands',         value: 350, color: '#0369a1' },
  { name: 'Temperate forest', value: 250, color: '#15803d' },
  { name: 'Heathland',        value: 170, color: '#b45309' },
];
  ...
  return (
    <div style={{ background: '#f9fafb', borderRadius: 10, padding: '24px 20px 16px', margin: '20px 0' }}>
      ...
      <div style={{ fontSize: 12, color: '#4b5563', marginBottom: 20 }}>
        Approximate total stock (soil + biomass), tCO&#x2082;eq / ha &middot; Temperate European conditions &middot; IPCC / EEA
      </div>
      <div style={{ width: '100%', height: 220 }}>
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={CARBON_DATA} layout="vertical" margin={{ top: 0, right: 70, left: 0, bottom: 0 }}>
            <CartesianGrid strokeDasharray="3 3" horizontal={false} stroke="#e5e7eb" />
            <XAxis type="number" tick={{ fontSize: 11, fill: '#374151' }} tickFormatter={v => `${v}`}
              label={{ value: 'tCO\u2082eq / ha', position: 'insideBottomRight', offset: -10, fontSize: 11, fill: '#4b5563' }} />
            <YAxis type="category" dataKey="name" width={145} tick={{ fontSize: 13, fill: '#374151' }} interval={0} />
            ...
            <Bar dataKey="value" radius={[0, 4, 4, 0]}
              ...
              {CARBON_DATA.map((entry, index) => <Cell key={index} fill={entry.color} />)}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
      </div>
...
// % of Belgium: natural ecosystems + "Other" (farmland ~55% + urban ~20% = ~75.2%)
const PIE_BELGIUM = [
  ...ECO_LIST.map(e => ({ name: e.name, value: e.belgiumPct, color: e.color })),
  { name: 'Other', value: 75.2, color: '#374151' },
];

const PIE_NATURAL = ECO_LIST.map(e => ({ name: e.name, value: e.naturalPct, color: e.color }));

function EcosystemPieChart() {
  const [view, setView] = useState<'belgium' | 'natural'>('belgium');
  const [openIdx, setOpenIdx] = useState<number | null>(null);

  const pieData = view === 'belgium' ? PIE_BELGIUM : PIE_NATURAL;

  const PieTooltip = ({ active, payload }: any) => {
    if (!active || !payload?.length) return null;
    return (
      <div style={{ background: '#1a1a1a', borderRadius: 8, padding: '8px 12px', fontSize: 13, color: '#fff' }}>
  ...
  return (
    <div style={{ margin: '20px 0' }}>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 20, flexWrap: 'wrap' as const }}>
        {([
          { key: 'belgium', label: '% of Belgium total' },
          { key: 'natural', label: '% of natural area' },
        ] as const).map(btn => (
          <button key={btn.key} onClick={() => setView(btn.key)} style={{
            padding: '6px 14px', borderRadius: 20,
            border: `2px solid ${view === btn.key ? '#15803d' : '#e5e7eb'}`,
            background: view === btn.key ? '#15803d' : '#fff',
            color: view === btn.key ? '#fff' : '#374151',
      ...
      {/* Pie + legend */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 28, flexWrap: 'wrap' as const, marginBottom: 8 }}>
          ...
          {pieData.map(d => (
            <div key={d.name} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <span style={{ fontSize: 13, color: '#374151' }}>
                {d.name}{' '}<span style={{ color: '#4b5563', fontWeight: 600 }}>({d.value}%)</span>
              </span>
            </div>
          ))}
        </div>
      ...
      {/* Other explanation — only in Belgium view */}
      {view === 'belgium' && (
        <p style={{ fontSize: 12, color: '#4b5563', marginBottom: 16, lineHeight: 1.55 }}>
          <strong>Other</strong> covers farmland (~55% of Belgium&apos;s territory) and urban areas (~20%).
          Together they dominate the landscape; this article focuses on the natural ecosystems only.
        </p>
      )}
      ...
      <div style={{ marginTop: 12 }}>
        <p style={{ fontSize: 13, fontWeight: 600, color: '#374151', marginBottom: 10 }}>
          Click an ecosystem to learn more:
        </p>
        {ECO_LIST.map((eco, i) => (
          <div key={eco.name} style={{ marginBottom: 6 }}>
            <button
              onClick={() => setOpenIdx(openIdx === i ? null : i)}
              style={{
                width: '100%', textAlign: 'left' as const, padding: '10px 14px',
                borderRadius: openIdx === i ? '8px 8px 0 0' : 8,
                border: `2px solid ${openIdx === i ? eco.color : '#e5e7eb'}`,
                background: openIdx === i ? `${eco.color}12` : '#f9fafb',
                cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 10,
                transition: 'all 0.15s', fontFamily: 'inherit',
              }}
            >
              <span style={{ width: 12, height: 12, borderRadius: 3, background: eco.color, flexShrink: 0, display: 'inline-block' }} />
              <span style={{ fontWeight: 700, fontSize: 14, color: openIdx === i ? eco.color : '#1a1a1a', flex: 1 }}>
                {eco.name}
              </span>
            ...
            {openIdx === i && (
              <div style={{
                ...
                color: '#374151', background: `${eco.color}08`,
                border: `2px solid ${eco.color}`, borderTop: 'none',
      ...
      >
        <div className="detail-header-inner" style={{ display: 'flex', alignItems: 'flex-start', gap: 32 }}>
          <div style={{ flex: 1 }}>
            ...
            <div className="header-eyebrow" style={{ color: '#86efac', marginTop: 16 }}>
              &#x1F33F;&nbsp; Nature &amp; Biodiversity
            </div>
            ...
            <p style={{ fontSize: 16, color: '#bbf7d0', maxWidth: 600, lineHeight: 1.6, margin: 0 }}>
              How nature works for us &mdash; and what it&apos;s worth.
            </p>
          </div>
          <img
            src="/images/learn/ecosystem-services.jpg"
      ...
      <div className="detail-body">
        ...
        <div className="detail-main">
          ...
          <SectionCard id="intro">
            ...
            <Para>
              An <strong>ecosystem</strong> is a community of living organisms &mdash; plants, animals,
              fungi, and microorganisms &mdash; interacting with each other and with their physical
            ...
            <Para>
              ...
              systems &mdash; directly and indirectly. The term sounds technical, but the concept is
              ...
              services make that invisible work visible &mdash; and in some cases, quantifiable.
            </Para>
          </SectionCard>
          ...
          <SectionCard id="ecosystems">
            ...
            <Para>
              ...
              Farmland covers around 55% of the country and urban areas a further 20% &mdash; together
            ...
            <Para style={{ marginTop: 16 }}>
              The chart below illustrates one of the most important &mdash; and least intuitive &mdash; facts
          ...
          <SectionCard id="value">
            ...
            <Para>
              ...
              and infinite. Pricing them sounds uncomfortable &mdash; nature has value beyond what
              markets can capture &mdash; but quantifying ecosystem services has become an important
            ...
            <Para>
              ...
              concluded that <strong>every &euro;1 invested in nature restoration generates between
              &euro;8 and &euro;38 in economic value.</strong>{' '}The wide range reflects genuine
              variation across ecosystem types: peatlands and coastal wetlands tend to score
              highest, due to their combined flood protection, carbon storage, and water
              purification value. Restoring a Scheldt salt marsh delivers different services
              at different scales than restoring a hay meadow in the Ardennes.
            </Para>
            ...
            <div style={{ background: 'linear-gradient(135deg, #f0fdf4, #dcfce7)', border: '1px solid #86efac', borderRadius: 12, padding: '24px 28px', margin: '20px 0', display: 'flex', alignItems: 'flex-start', gap: 20 }}>
              ...
              <div>
                <div style={{ fontSize: 28, fontWeight: 800, color: '#14532d', lineHeight: 1.1 }}>
                  &euro;8 &ndash; &euro;38
                </div>
                <div style={{ fontSize: 14, color: '#166534', fontWeight: 600, margin: '4px 0 8px' }}>
                  returned for every &euro;1 invested in nature restoration
                </div>
                <div style={{ fontSize: 13, color: '#4b7a52', lineHeight: 1.5 }}>
                  ...
                  Depending on ecosystem type &mdash; peatlands and wetlands deliver the highest returns.
                </div>
              </div>
            </div>
            ...
            <NoteBox>
              ...
              an environmental problem &mdash; it is a financial risk.
            </NoteBox>
          </SectionCard>
          ...
          <SectionCard id="tool">
            ...
            <Para>
              For anyone working on a nature project in Flanders &mdash; whether a government agency,
              an NGO, a landowner, or a spatial planner &mdash; the{' '}
            ...
            <div style={{ background: '#f9fafb', border: '1px solid #e5e7eb', borderRadius: 10, padding: '20px 24px', margin: '16px 0' }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: 16, flexWrap: 'wrap' as const }}>
                {[
                  { title: 'What it does', body: 'Quantifies and monetises the ecosystem services delivered by a piece of land \u2014 covering at least 18 services from carbon storage and flood regulation to recreation and air quality.' },
                  { title: 'How to use it', body: 'Draw your study area and proposed land use changes on a map. The tool automatically calculates the before-and-after value of ecosystem services. No data collection needed \u2014 everything is loaded from existing Flemish datasets.' },
                  { title: "What it's for", body: 'The results feed directly into a social cost-benefit analysis (MKBA), comparing the value of a nature project with alternative land uses in a common unit \u2014 euros. Useful for governments, developers, and nature organisations alike.' },
                ].map(item => (
                  <div key={item.title} style={{ flex: 1, minWidth: 200 }}>
                    <div style={{ fontWeight: 700, fontSize: 15, color: '#1a1a1a', marginBottom: 6 }}>{item.title}</div>
                    <p style={{ fontSize: 14, lineHeight: 1.7, color: '#374151', margin: 0 }}>{item.body}</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            <Para>
              ...
              <a href="https://www.natuurwaardeverkenner.be" target="_blank" rel="noopener noreferrer"
          ...
          <SectionCard id="sources">
            ...
            <div style={{ display: 'flex', flexDirection: 'column' as const, gap: 10 }}>
              {[
                { label: 'Millennium Ecosystem Assessment (2005) \u2014 Ecosystems and Human Well-being: Synthesis', url: 'https://www.millenniumassessment.org/documents/document.356.aspx.pdf' },
                { label: 'TEEB \u2014 The Economics of Ecosystems and Biodiversity: Mainstreaming the Economics of Nature', url: 'https://teebweb.org/publications/teeb-for/' },
                { label: 'European Commission (2022) \u2014 Impact Assessment for the EU Nature Restoration Law (\u20ac8\u2013\u20ac38 return per \u20ac1 invested)', url: 'https://environment.ec.europa.eu/publications/nature-restoration-law_en' },
                { label: 'IPCC / EEA \u2014 Carbon stocks in European terrestrial ecosystems', url: 'https://www.eea.europa.eu/themes/biodiversity' },
                { label: 'European Central Bank (2021) \u2014 The nature of the risk: biodiversity loss and the implications for financial stability', url: 'https://www.ecb.europa.eu/press/research-publications/resbull/2021/html/ecb.rb210712~b197b8bb32.en.html' },
                { label: 'INBO / Agentschap Natuur en Bos \u2014 Ecosystem service quantification for Flemish protected areas', url: 'https://www.inbo.be' },
                { label: 'Sigma Plan \u2014 Flood protection and tidal marsh restoration along the Scheldt estuary', url: 'https://www.sigmaplan.be' },
                { label: 'Natuurwaardeverkenner \u2014 VITO / University of Antwerp, free ecosystem service valuation tool for Flanders', url: 'https://www.natuurwaardeverkenner.be' },
              ].map(link => (
