...
// ── Mammal Biomass Treemap ────────────────────────────────────────────────────

const BIOMASS_DATA = [
  { name: 'Livestock',      size: 60, emoji: '🐄', examples: 'Cattle, pigs, sheep, goats' },
  { name: 'Humans',         size: 36, emoji: '🧑', examples: '8 billion people' },
  { name: 'Wild mammals',   size: 4,  emoji: '🐺', examples: 'Every elephant, whale, wolf, bat & rodent' },
];

const BIOMASS_COLORS: Record<string, string> = {
  Livestock:     '#92400e',
  Humans:        '#1e40af',
  'Wild mammals':'#14532d',
};

const BIOMASS_BG: Record<string, string> = {
  Livestock:     '#fef3c7',
  Humans:        '#dbeafe',
  'Wild mammals':'#dcfce7',
};

function BiomassContent(props: any) {
  const { x, y, width, height, name } = props;
  if (!width || !height || width < 20 || height < 20) return null;
  const item = BIOMASS_DATA.find((d) => d.name === name);
  const size = item?.size ?? 0;
  const showEmoji    = width > 80  && height > 50;
  const showExamples = width > 140 && height > 90;
  return (
    <g>
      <rect
        x={x + 2} y={y + 2} width={width - 4} height={height - 4}
        fill={BIOMASS_BG[name]} rx={8}
        stroke={BIOMASS_COLORS[name]} strokeWidth={2}
      />
      {showEmoji && (
        <text x={x + 16} y={y + 30} fontSize={22} dominantBaseline="middle">
          {item?.emoji}
        </text>
      )}
      <text
        x={x + 16} y={showEmoji ? y + 56 : y + height / 2 - 8}
        fontSize={Math.min(15, width / 8)} fontWeight={700}
        fill={BIOMASS_COLORS[name]}
      >
        {name}
      </text>
      <text
        x={x + 16} y={showEmoji ? y + 76 : y + height / 2 + 12}
        fontSize={Math.min(22, width / 5)} fontWeight={800}
        fill={BIOMASS_COLORS[name]}
      >
        {size}%
      </text>
      {showExamples && (
        <text x={x + 16} y={y + 104} fontSize={12} fill={BIOMASS_COLORS[name]} opacity={0.8}>
          {item?.examples}
        </text>
      )}
    </g>
...
function BiomassTooltip({ active, payload }: any) {
  ...
  const item = BIOMASS_DATA.find((i) => i.name === d.name);
  return (
    <div style={{ background: '#1a1a1a', borderRadius: 8, padding: '10px 14px', fontSize: 13, color: '#fff', boxShadow: '0 4px 12px rgba(0,0,0,0.3)' }}>
      <div style={{ fontWeight: 700, marginBottom: 4 }}>{item?.emoji} {d.name}</div>
      <div style={{ color: '#d1d5db' }}>{d.size}% of all mammal biomass</div>
      <div style={{ color: '#9ca3af', fontSize: 12, marginTop: 4 }}>{item?.examples}</div>
    </div>
  );
}
    ...
    <div style={{ background: '#f9fafb', borderRadius: 10, padding: '24px 20px 16px', margin: '20px 0' }}>
      ...
      <div style={{ width: '100%', height: 300 }}>
        <ResponsiveContainer width="100%" height="100%">
          {/* @ts-expect-error Recharts Treemap content prop typing is overly strict */}
          <Treemap data={BIOMASS_DATA} dataKey="size" aspectRatio={4 / 3} content={<BiomassContent />}>
