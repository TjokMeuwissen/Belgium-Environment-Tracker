...
function SupplyConcentrationChart() {
  const data = [
    { material: 'Magnesium',  country: 'China',     share: 87, color: '#ef4444' },
    { material: 'Gallium',    country: 'China',     share: 80, color: '#ef4444' },
    { material: 'Cobalt',     country: 'DRC',       share: 70, color: '#f97316' },
    { material: 'Graphite',   country: 'China',     share: 65, color: '#ef4444' },
    { material: 'REE mining', country: 'China',     share: 60, color: '#ef4444' },
    { material: 'Lithium',    country: 'Australia', share: 47, color: '#3b82f6' },
    { material: 'Indium',     country: 'China',     share: 57, color: '#ef4444' },
    { material: 'REE refining',country:'China',     share: 85, color: '#ef4444' },
  ].sort((a, b) => b.share - a.share);

  return (
    <div style={{ background: '#f9fafb', borderRadius: 10, padding: '20px', marginBottom: 16 }}>
      <p style={{ fontFamily: 'Roboto, sans-serif', fontWeight: 700, fontSize: '0.78rem', color: '#6b7280', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 4 }}>
        Top producer share of global supply (%)
      </p>
      <p style={{ fontSize: '0.78rem', color: '#9ca3af', marginBottom: 16 }}>
        Source: European Commission Critical Raw Materials study 2023 / USGS Mineral Commodity Summaries 2024
      </p>
      <ResponsiveContainer width="100%" height={260}>
        <BarChart data={data} layout="vertical" margin={{ left: 20, right: 50, top: 0, bottom: 0 }}>
          ...
          <Tooltip formatter={(v: number) => [`${v}%`, 'Top producer share']} labelFormatter={(_: string, payload: {payload?: {country?:string}}[]) => payload?.[0]?.payload?.country ?? ''} />
          <Bar dataKey="share" radius={[0, 4, 4, 0]}>
            {data.map((entry, i) => <Cell key={i} fill={entry.color} />)}
            <LabelList dataKey="share" position="right" style={{ fontSize: 11, fill: '#374151' }} formatter={(v: number) => `${v}%`} />
          </Bar>
        </BarChart>
      </ResponsiveContainer>
      <div style={{ display: 'flex', gap: 16, marginTop: 10 }}>
        ...
        <div className="detail-main">
          ...
          <SectionCard id="green-hunger">
            ...
            <Para>
              ...
              neodymium and dysprosium for their magnets; hydrogen electrolysers need platinum-group metals; and the
              electronics controlling all of these technologies depend on gallium, germanium and tantalum. The green
              transition is, at its material foundation, a massive reorientation of global mining and metallurgy.
            </Para>
          </SectionCard>
