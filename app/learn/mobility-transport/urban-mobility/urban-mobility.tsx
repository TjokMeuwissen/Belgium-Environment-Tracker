...
// ── Data ──────────────────────────────────────────────────────────────────────
// Source: Eurostat tran_hv_psmod, 2021
const modalData = [
  { country: 'Belgium',     car: 85,   bus: 7,  rail: 8  },
  { country: 'EU average',  car: 70.6, bus: 10.2, rail: 8.5 },
  { country: 'Netherlands', car: 75,   bus: 4,  rail: 10 },
  { country: 'Germany',     car: 80,   bus: 7,  rail: 9  },
  { country: 'France',      car: 83,   bus: 6,  rail: 10 },
  { country: 'Denmark',     car: 78,   bus: 9,  rail: 10 },
];
    ...
    <>
      <ResponsiveContainer width="100%" height={260}>
        ...
        >
          ...
          <Bar dataKey="rail" name="Rail" stackId="a" fill="#16a34a" radius={[0, 4, 4, 0]} />
        </BarChart>
      </ResponsiveContainer>
      {/* Legend */}
      ...
      <ResponsiveContainer width="100%" height={200}>
        ...
        >
          ...
          <YAxis
            ...
            tickFormatter={v => `${v}`}
            width={36}
            label={{ value: '\u03bcg/m\u00b3', angle: -90, position: 'insideLeft', offset: 12, style: { fontSize: 10, fill: '#6b7280' } }}
          />
          <Tooltip content={<NO2Tooltip />} />
          {/* EU limit 40 µg/m³ */}
          <ReferenceLine y={40} stroke="#ef4444" strokeDasharray="4 3" label={{ value: 'EU limit 40', position: 'right', fontSize: 10, fill: '#ef4444' }} />
          ...
          <Bar dataKey="no2" name="NO\u2082 (Arts-Lois)" fill="#3b82f6" radius={[4, 4, 0, 0]}>
            <LabelList dataKey="no2" position="top" style={{ fontSize: 10, fill: '#374151' }} formatter={(v: number) => `${v}`} />
          </Bar>
        </BarChart>
      </ResponsiveContainer>
...
// ── Ghent before/after bar ────────────────────────────────────────────────────
function GhentBar() {
  const years = [
    {
      label: '2012 (before)',
      car: 54, cycling: 22, pt: 9, walk: 14,
    },
    {
      label: '2021 (after)',
      car: 40, cycling: 34, pt: 13, walk: 13,
    },
  ];
  const modes = [
    { key: 'car',     label: 'Car',            color: '#ef4444' },
    { key: 'cycling', label: 'Cycling',        color: '#16a34a' },
    { key: 'pt',      label: 'Public transport', color: '#3b82f6' },
    { key: 'walk',    label: 'Walking',         color: '#f97316' },
  ];

  return (
    <div style={{ marginBottom: 16 }}>
      {years.map(yr => (
        <div key={yr.label} style={{ marginBottom: 14 }}>
          <p style={{
            fontFamily: 'Roboto, sans-serif', fontWeight: 700,
            fontSize: '0.82rem', color: '#374151', marginBottom: 6,
          }}>
            {yr.label}
          </p>
          <div style={{ height: 32, borderRadius: 6, overflow: 'hidden', display: 'flex', boxShadow: '0 1px 4px rgba(0,0,0,0.1)' }}>
            {modes.map(m => {
              const pct = (yr as any)[m.key];
              return (
                <div key={m.key} style={{
      ...
      <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap' as const, marginTop: 6 }}>
        {modes.map(m => (
          <div key={m.key} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            <span style={{ fontFamily: 'Roboto, sans-serif', fontSize: '0.8rem', color: '#374151' }}>{m.label}</span>
          </div>
        ))}
      </div>
