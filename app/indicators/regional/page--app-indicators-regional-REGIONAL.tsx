...
interface RegInd {
  ...
  explainer?: string;  // optional warning box below chart
}
...
const INDICATORS: RegInd[] = [
  {
    ...
    isTotal: true, forceMax: 100,
    note: 'Why is Brussels so low? The Brussels-Capital Region is a small, densely urban territory (161 km\u00b2) with virtually no industry, agriculture or energy generation. Almost all electricity (~95%) is produced outside the region. Emissions come primarily from buildings (heating) and road transport. For comparison, the Brussels population (1.2M) is only ~10% of Belgium\'s total. Brussels derived as residual (97.9 \u2212 65.4 \u2212 29.4). Flanders: VMM open definition. Wallonia: AWAC excl. forestry. Sources: VMM (2025), AWAC (2025).',
  },
  {
    ...
    forceMax: 12,
    explainer: 'Why is Brussels so low? This indicator counts only emissions physically occurring within a region\'s borders. Brussels has no power plants, heavy industry or agriculture, so its territorial total is very small relative to 1.2 million inhabitants. Brussels residents\u2019 actual climate impact \u2014 including electricity generated elsewhere and imported goods \u2014 is estimated to be much closer to the Belgian average.',
    note: 'Sources: VMM, AWAC, IWEPS (2025).',
  },
  {
    ...
    forceMax: 15,
    note: 'Wallonia holds ~91% of Belgian organic area. Flanders has more intensive conventional agriculture with limited conversion to organic. Brussels has negligible agricultural area (N/A). Sources: Statbel (2023), BIOWALLONIE, Dep. Landbouw & Visserij.',
  },
  {
  ...
  return (
    <div style={{ background: '#fff', borderRadius: 12, boxShadow: '0 2px 12px rgba(0,0,0,0.07)', overflow: 'hidden', display: 'grid', gridTemplateColumns: '5px 1fr', marginBottom: 16 }}>
      ...
      <div style={{ padding: '20px 24px 16px' }}>
        ...
        {/* Explainer warning box */}
        {ind.explainer && (
          <div style={{ marginTop: 14, background: '#fffbeb', border: '1px solid #fde68a', borderRadius: 8, padding: '12px 14px', display: 'flex', gap: 10, alignItems: 'flex-start' }}>
            <span style={{ fontSize: '1rem', flexShrink: 0, lineHeight: 1.4 }}>⚠️</span>
            <p style={{ fontSize: '0.83rem', color: '#374151', lineHeight: 1.65, margin: 0 }}>
              {ind.explainer}
            </p>
          </div>
        )}
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)' }}>
      ...
      <div style={{ background: '#1a1a1a', color: 'white', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ maxWidth: 900, margin: '0 auto', padding: '40px 48px 36px', textAlign: 'center' }}>
          ...
          <p style={{ color: '#b0b0b0', fontSize: '0.95rem', marginTop: 8, maxWidth: 520, margin: '8px auto 0' }}>
          ...
          {/* Region legend with flag SVGs */}
          <div style={{ display: 'flex', gap: 28, marginTop: 20, flexWrap: 'wrap', alignItems: 'center', justifyContent: 'center' }}>
            {[
              { label: 'Flanders', flag: '/images/flag-flanders.svg' },
              { label: 'Wallonia',  flag: '/images/flag-wallonia.svg' },
              { label: 'Brussels', flag: '/images/flag-brussels.svg' },
            ].map(r => (
              <span key={r.label} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8, fontSize: '0.85rem', color: '#d1d5db' }}>
                <img src={r.flag} alt={`${r.label} flag`}
                  style={{ width: 84, height: 'auto', borderRadius: 3, display: 'block', border: '1px solid rgba(255,255,255,0.15)' }}
                />
                {r.label}
              </span>
            ))}
      ...
      {/* Sidebar + content */}
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '32px 24px 64px', display: 'flex', gap: 32, alignItems: 'flex-start' }}>

        {/* Sidebar — uses existing climate-sidebar CSS */}
        <div
          className="climate-sidebar"
          style={{ '--topic-color': TOPIC_COLOR[activeFilter] ?? '#374151' } as React.CSSProperties}
        >
          <div className="sidebar-group-label">Topics</div>
          {[{ id: 'all', label: 'All indicators' }, ...topicsPresent.map(t => ({ id: t, label: TOPIC_LABEL[t] }))].map(f => (
            <button
              key={f.id}
              onClick={() => setActiveFilter(f.id)}
              className={`sidebar-link${activeFilter === f.id ? ' active' : ''}`}
              style={{
