...
function EUMap({ indicator, topicColor }: { indicator: Indicator; topicColor: string }) {
  ...
  const getFillColor = (code: string): string => {
    if (!EU27.has(code)) return '#5a6270'; // non-EU → dark charcoal grey // non-EU → visible grey
    const entry = rankMap[code];
    if (!entry) return '#dde1e6'; // EU, no data → very light grey           // EU but no data → slightly darker grey
