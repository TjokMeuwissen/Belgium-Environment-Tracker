...
function EUMap({ indicator, topicColor }: { indicator: Indicator; topicColor: string }) {
  ...
  const getFillColor = (code: string): string => {
    if (!EU27.has(code)) return '#e5e7eb'; // non-EU → light grey
    const entry = rankMap[code];
    if (!entry) return '#d1d5db';           // EU but no data
    ...
    <div style={{ position: 'relative' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 10 }}>
        ...
        <span style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: '0.72rem', color: '#6b7280' }}>
          <span style={{ width: 12, height: 12, borderRadius: 2, background: '#d1d5db', flexShrink: 0, display: 'inline-block' }} /> No data
        </span>
        <span style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: '0.72rem', color: '#6b7280' }}>
          <span style={{ width: 12, height: 12, borderRadius: 2, background: '#e5e7eb', flexShrink: 0, display: 'inline-block' }} /> Non-EU
        </span>
      </div>
    </div>
  );
...
function EUBarChart({ indicator }: { indicator: Indicator }) {
  ...
  const chartH = Math.max(380, n * 23 + 80);
      ...
      <div style={{ width: '100%', height: chartH }}>
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={sorted} layout="vertical" margin={{ top: 12, right: 70, left: 0, bottom: 32 }}>
            ...
            <YAxis type="category" dataKey="name" width={112} tick={{ fontSize: 10.5, fill: '#374151' }} tickLine={false} axisLine={false} />
    ...
    <main>
      ...
      <section style={{ maxWidth: 1800, margin: '0 auto', padding: '32px 40px 64px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 12, gap: 6 }}>
          {(['bar', 'map'] as const).map(mode => (
            <button key={mode} onClick={() => setViewMode(mode)} style={{
              padding: '5px 14px', borderRadius: 6, fontSize: '0.82rem', fontWeight: viewMode === mode ? 700 : 400,
              border: `1.5px solid ${viewMode === mode ? topic.color : '#e5e7eb'}`,
              background: viewMode === mode ? topic.color : '#fff',
              color: viewMode === mode ? '#fff' : '#6b7280',
              cursor: 'pointer', transition: 'all 0.15s',
            }}>
              {mode === 'bar' ? '▬ Bar chart' : '🗺 Map'}
            </button>
          ))}
        </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16, flexWrap: 'wrap' }}>
            ...
            {viewMode === 'bar' && (
              <span style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 14, fontSize: '0.78rem', color: '#6b7280' }}>
                ...
                <span style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                  <span style={{ width: 16, height: 2, borderTop: `2px dashed ${EU_COLOR}`, display: 'inline-block' }} /> EU-27 average
                </span>
              </span>
            )}
          </div>
