...
function KeyStats({ indicator, topicColor }: { indicator: Indicator; topicColor: string }) {
  ...
  const isGood = rank > 0 && rank <= Math.ceil(n / 3);
  ...
  const rankColor     = isGood ? '#16a34a' : '#dc2626';
  ...
  return (
    <main>
      ...
      {topic.indicators.length > 1 && (
        <div style={{ background: '#f9fafb', borderBottom: '1px solid #e5e7eb', padding: '10px 24px', display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          {topic.indicators.map(ind => {
            const active = ind.key === activeInd;
            return (
              <button key={ind.key} onClick={() => setActiveInd(ind.key)} style={{
                padding: '5px 14px', borderRadius: 6, fontSize: '0.82rem', fontWeight: active ? 600 : 400,
