...
function EUMap({ indicator, topicColor }: { indicator: Indicator; topicColor: string }) {
  ...
  const getFillColor = (code: string): string => {
    if (!EU27.has(code)) return '#c8c8c8'; // non-EU → visible grey
    const entry = rankMap[code];
    if (!entry) return '#b0b0b0';           // EU but no data → slightly darker grey
    ...
    <div style={{ position: 'relative' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 10 }}>
        ...
        <span style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: '0.72rem', color: '#6b7280' }}>
          <span style={{ width: 12, height: 12, borderRadius: 2, background: '#b0b0b0', flexShrink: 0, display: 'inline-block' }} /> No data
        </span>
        <span style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: '0.72rem', color: '#6b7280' }}>
          <span style={{ width: 12, height: 12, borderRadius: 2, background: '#c8c8c8', flexShrink: 0, display: 'inline-block' }} /> Non-EU
        </span>
      </div>
    </div>
  );
      ...
      <div style={{ width: '100%', height: chartH }}>
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={sorted} layout="vertical" margin={{ top: 12, right: 70, left: 0, bottom: 32 }}>
            ...
            <YAxis type="category" dataKey="name" width={116} tick={<CustomYTick />} tickLine={false} axisLine={false} />
