...
const EF = {
  ...
  car:{
    petrol:  { small:0.152, medium:0.179, large:0.248 },
    diesel:  { small:0.140, medium:0.164, large:0.217 },
    lpg:     { small:0.133, medium:0.162, large:0.228 },
    cng:     { small:0.100, medium:0.130, large:0.170 },
    phev:    { small:0.077, medium:0.100, large:0.163 },
    electric:{ small:0.008, medium:0.012, large:0.020 },
...
interface CalcState {
  ...
  commuteMode:'single'|'multiple'; commuteSingleMode:string; commuteCarSize:'small'|'medium'|'large';
  commuteModes:{mode:string;share:number}[];
  carpoolCar:boolean; carpoolPeople:number;
  otherTrips:{mode:string;tripsPerWeek:number;kmOneWay:number;carSize:'small'|'medium'|'large'}[];
...
const DEFAULT:CalcState = {
  ...
  commuteMode:'single', commuteSingleMode:'petrol', commuteCarSize:'medium',
...
function getCarEF(mode:string, size:string):number {
  const entry = EF.car[mode];
  if (entry && typeof entry === 'object') return entry[size] ?? entry.medium ?? 0;
  return 0;
}

function getPTEF(mode:string):number {
  return EF.transport[mode] ?? 0;
}

function getEF(mode:string, carSize:string='medium'):number {
  if (EF.car[mode]) return getCarEF(mode, carSize);
  return getPTEF(mode);
}

const CAR_MODES = new Set(['petrol','diesel','electric','phev','lpg','cng']);
...
function calc(s:CalcState) {
  ...
  if (s.commuteMode==='single') {
    const ef = getEF(s.commuteSingleMode, s.commuteCarSize);
    ...
    for (const cm of s.commuteModes) {
      const ef = getEF(cm.mode, s.commuteCarSize);
      const div = CAR_MODES.has(cm.mode) ? carpoolDiv : 1;
      transport += s.commuteKm*2*workDays*(ef/div)*(cm.share/100);
    }
  }
  for (const t of s.otherTrips) transport += t.tripsPerWeek*52*t.kmOneWay*2*getEF(t.mode, t.carSize);
...
function CarSizePicker({ val, onChange }: { val:string; onChange:(v:'small'|'medium'|'large')=>void }) {
  return (
    <div style={{display:'flex',gap:8,marginTop:12,alignItems:'center'}}>
      <span style={{fontSize:13,color:'#4b5563',flexShrink:0}}>Car size:</span>
      {([
        {id:'small', label:'🚗 Small', sub:'A/B segment'},
        {id:'medium',label:'🚙 Medium',sub:'C/D segment'},
        {id:'large', label:'🚐 Large', sub:'E + SUV'},
      ] as {id:'small'|'medium'|'large';label:string;sub:string}[]).map(o=>(
        <button key={o.id} onClick={()=>onChange(o.id)}
          style={{padding:'9px 20px',borderRadius:8,border:`2px solid ${val===o.id?ACCENT:'#e5e7eb'}`,background:val===o.id?ACCENT_BG:'#fff',cursor:'pointer',fontSize:14,fontWeight:600,color:val===o.id?ACCENT:'#374151',lineHeight:1.4,textAlign:'center',minWidth:100}}>
          <div>{o.label}</div>
          <div style={{fontSize:12,color:val===o.id?ACCENT:'#9ca3af',fontWeight:400}}>{o.sub}</div>
        </button>
      ))}
    </div>
  );
        ...
        <>
          ...
          <div style={{background:'#f9fafb',borderRadius:10,padding:'16px 20px',marginBottom:20}}>
            ...
            {[
              ...
              {label:'Belgian average (consumption-based)', value:15.7, color:'#f97316'},
    ...
    <div id="methodology" style={{background:'#fff',borderRadius:12,boxShadow:'0 1px 4px rgba(0,0,0,0.08)',padding:'28px 32px',marginBottom:20}}>
      ...
      {([
        ...
        },
        {
          ...
          items:[
            { label:'Car emission factors per km (well-to-wheel, incl. upstream fuel production) — size-differentiated: small (A/B), medium (C/D), large/SUV', url:'https://base-empreinte.ademe.fr', source:'ADEME Base Empreinte (WTW per-liter factors) × real-world consumption by segment (EEA / ICCT 2023)' },
            { label:'Petrol: 2.76 kg CO₂eq/L · Diesel: 3.10 kg CO₂eq/L · LPG: 1.90 kg CO₂eq/L (all WTW, ADEME Base Empreinte)', url:'https://base-empreinte.ademe.fr', source:'ADEME Base Empreinte' },
            { label:'Electric vehicle (BEV) transport factor covers battery manufacturing lifecycle only (0.008–0.020 kg CO₂eq/km depending on battery size: 40 kWh small / 60 kWh medium / 100 kWh large × 40 kg CO₂eq/kWh ÷ 200,000 km lifetime — JRC). Grid electricity for charging is counted in the Electricity section (via the EV charging kWh input), where it can also be offset by solar panels. This avoids double-counting grid electricity in both transport and electricity.', url:null, assumption:'Split TTW + electricity approach. PHEV uses full WTW EF since charging electricity is not separately entered.' },
    ...
    <div style={{minHeight:'100vh',background:'var(--bg,#f4f4f2)'}}>
      ...
      <div style={{background:'linear-gradient(135deg,#052e16 0%,#14532d 50%,#15803d 100%)',padding:'40px 0 36px'}}>
        <div style={{maxWidth:1100,margin:'0 auto',padding:'0 24px 0 calc(24px + 190px + 32px)'}}>
          ...
          <h1 style={{fontFamily:'Roboto,sans-serif',fontSize:'clamp(1.8rem,4vw,2.6rem)',fontWeight:900,color:'#fff',margin:'0 0 10px',lineHeight:1.1,letterSpacing:'-0.02em'}}>Calculators</h1>
          <p style={{fontSize:16,color:'#bbf7d0',maxWidth:600,lineHeight:1.6,margin:'0 0 20px'}}>Two tools to understand your environmental footprint.</p>
          {/* Tab switcher */}
          <div style={{display:'flex',gap:4}}>
            {[
              {href:'/calculator',     label:'🌿 Carbon Footprint',   active:true },
              {href:'/crm-calculator', label:'⚙️ Critical Materials', active:false},
            ].map(tab=>(
              <Link key={tab.href} href={tab.href}
                style={{padding:'8px 20px',borderRadius:'8px 8px 0 0',textDecoration:'none',fontSize:13,fontWeight:700,
                  ...
                  color:tab.active?'#15803d':'#bbf7d0',transition:'all 0.15s'}}>
                {tab.label}
              </Link>
            ))}
          </div>
        ...
        <div style={{flex:1,minWidth:0}}>
          ...
          <div id="disclaimer" style={{background:'#fff',borderRadius:12,boxShadow:'0 1px 4px rgba(0,0,0,0.08)',padding:'22px 32px',marginBottom:20}}>
            <div style={{background:ACCENT_BG,border:`1px solid ${ACCENT_BORDER}`,borderRadius:10,padding:'16px 20px'}}>
              <div style={{fontWeight:700,fontSize:14,color:'#14532d',marginBottom:12}}>ℹ️ About this tool</div>
              <ul style={{listStyle:'none',padding:0,margin:0,display:'flex',flexDirection:'column',gap:10}}>
                <li style={{display:'flex',gap:10,fontSize:13,color:'#374151',lineHeight:1.65}}>
                  <span style={{color:ACCENT,fontWeight:700,flexShrink:0,marginTop:1}}>▸</span>
                  <span>
                    <strong>New to carbon footprints?</strong> The{' '}
                    <a href="/learn/climate-energy/carbon-footprint" style={{color:ACCENT,fontWeight:600,textDecoration:'underline'}}>
                      Carbon Footprint learn page
                    </a>
                    {' '}explains what a carbon footprint is, the difference between production-based and consumption-based accounting, and what Belgium&apos;s national figures actually represent.
                  </span>
                </li>
                <li style={{display:'flex',gap:10,fontSize:13,color:'#374151',lineHeight:1.65}}>
                  <span style={{color:ACCENT,fontWeight:700,flexShrink:0,marginTop:1}}>▸</span>
                  <span>
                    <strong>This tool calculates a carbon footprint per person</strong>, not per household. Certain impacts — like heating and electricity — are divided by the number of people in your household. Enter your household size first so these are allocated correctly.
                  </span>
                </li>
                <li style={{display:'flex',gap:10,fontSize:13,color:'#374151',lineHeight:1.65}}>
                  <span style={{color:ACCENT,fontWeight:700,flexShrink:0,marginTop:1}}>▸</span>
                  <span>
                    <strong>Work through each section</strong> — 🏠 Housing &amp; Heating, ⚡ Electricity, 🍽️ Food &amp; diet, 🚗 Transport, ✈️ Flights, 🛍️ Shopping, ♻️ Waste, and 🐾 Pets. You can fill in as many or as few as are relevant to you.
                  </span>
                </li>
                <li style={{display:'flex',gap:10,fontSize:13,color:'#374151',lineHeight:1.65}}>
                  <span style={{color:ACCENT,fontWeight:700,flexShrink:0,marginTop:1}}>▸</span>
                  <span>
                    <strong>See your results</strong> at the bottom of the page — a breakdown by category and a comparison against the Belgian average and climate targets. Your result will be lower than Belgium&apos;s published national average; this is expected and explained in the methodology note.
                  </span>
                </li>
                <li style={{display:'flex',gap:10,fontSize:13,color:'#374151',lineHeight:1.65}}>
                  <span style={{color:ACCENT,fontWeight:700,flexShrink:0,marginTop:1}}>▸</span>
                  <span>
                    <strong>More detail on sources and assumptions</strong> is in the{' '}
                    <a href="#methodology" style={{color:ACCENT,fontWeight:600,textDecoration:'underline'}}
                      onClick={e=>{e.preventDefault();document.getElementById('methodology')?.scrollIntoView({behavior:'smooth'});}}>
                      methodological note
                    </a>
                    {' '}at the bottom of this page. All calculations run in your browser — <strong>no personal data is sent to any server.</strong>
                  </span>
                </li>
              </ul>
            </div>
          ...
          <div style={{background:'#fff',borderRadius:12,boxShadow:'0 1px 4px rgba(0,0,0,0.08)',padding:'18px 32px',marginBottom:20,display:'flex',alignItems:'center',gap:20}}>
            <div style={{display:'flex',alignItems:'center',gap:6,flexShrink:0}}>
              <div style={{fontSize:14,fontWeight:600,color:'#374151'}}>People in your household:</div>
              <span style={{position:'relative',display:'inline-flex',alignItems:'center'}} className="hhinfo-wrap">
                <svg width="15" height="15" viewBox="0 0 16 16" fill="none" style={{cursor:'pointer',flexShrink:0,opacity:0.5}}>
                  <circle cx="8" cy="8" r="7.5" stroke="#374151" strokeWidth="1"/>
                  <circle cx="8" cy="5.5" r="1" fill="#374151"/>
                  <rect x="7.25" y="7.5" width="1.5" height="4" rx="0.75" fill="#374151"/>
                </svg>
                <style>{`.hhinfo-wrap:hover .hhinfo-tip{display:block!important}`}</style>
                <span className="hhinfo-tip" style={{
                  display:'none',position:'absolute',bottom:'120%',left:'50%',
                  transform:'translateX(-50%)',background:'#1a1a1a',color:'#f9fafb',
                  borderRadius:8,padding:'10px 14px',fontSize:12,lineHeight:1.55,
                  width:260,zIndex:100,boxShadow:'0 4px 16px rgba(0,0,0,0.2)',
                  pointerEvents:'none',fontWeight:400,
                }}>
                  This tool calculates a carbon footprint <strong style={{color:'#fff'}}>per person</strong>, not per household. Your household size is used to correctly allocate shared impacts — heating, electricity and shared appliances are divided equally among the people living in your home.
                </span>
              </span>
            </div>
            ...
            <div style={{marginTop:16}}>
              ...
              {s.commuteMode==='single'
                ? <>
                    ...
                    {/* Car size — only for car modes */}
                    {CAR_MODES.has(s.commuteSingleMode) && (
                      <CarSizePicker val={s.commuteCarSize} onChange={v=>upd('commuteCarSize',v)} />
                    )}
                ...
                : (
                  <div style={{background:'#f9fafb',borderRadius:8,padding:'14px 16px'}}>
                    ...
                    {/* Car size — only shown when any car mode has share > 0 */}
                    {s.commuteModes.some(m=>m.share>0 && CAR_MODES.has(m.mode)) && (
                      <div style={{paddingTop:10,borderTop:'1px solid #e5e7eb',marginTop:4}}>
                        <CarSizePicker val={s.commuteCarSize} onChange={v=>upd('commuteCarSize',v)} />
                      </div>
                    )}
            ...
            <div style={{marginTop:24}}>
              ...
              {s.otherTrips.map((t,idx)=>(
                <div key={idx} style={{background:'#f9fafb',borderRadius:8,padding:'12px 14px',marginBottom:8,display:'flex',gap:12,flexWrap:'wrap',alignItems:'flex-end'}}>
                  ...
                  {CAR_MODES.has(t.mode) && (
                    <div style={{width:'100%',borderTop:'1px solid #e5e7eb',paddingTop:8,marginTop:2}}>
                      <CarSizePicker val={t.carSize} onChange={v=>{const trips=[...s.otherTrips];trips[idx]={...trips[idx],carSize:v};upd('otherTrips',trips);}} />
                    </div>
                  )}
                </div>
              ))}
              <button onClick={()=>upd('otherTrips',[...s.otherTrips,{mode:'petrol',tripsPerWeek:1,kmOneWay:10,carSize:'medium'}])} style={{padding:'8px 16px',borderRadius:8,border:`2px dashed ${ACCENT}`,background:'transparent',color:ACCENT,cursor:'pointer',fontSize:13,fontWeight:600,width:'100%',marginTop:4}}>+ Add a trip</button>
            </div>
          </Card>
