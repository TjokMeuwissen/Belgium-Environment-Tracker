...
const EF = {
  ...
  electricity:0.185,          // kg CO₂eq/kWh (EEA lifecycle, including upstream)
...
function calc(s:CalcState) {
  ...
  const selfConsumed = Math.min(solarKwp*EF.solarYieldPerKwp*0.3, totalElecKwh/hh);
  const gridPerPerson = Math.max(0, totalElecKwh/hh - selfConsumed);
  ...
  if (s.commuteMode==='single') {
    transport = s.commuteKm*2*workDays*getEF(s.commuteSingleMode);
  } else {
    for (const cm of s.commuteModes) transport += s.commuteKm*2*workDays*getEF(cm.mode)*(cm.share/100);
  }
      ...
      {[
        {
          ...
          text:'Emission factors per kWh of delivered heat: IPCC 2006 Guidelines (combustion) + upstream supply chain losses (~9% for natural gas). Insulation benchmarks from Energuide.be and EPB regional statistics. Home size defaults from Statbel housing statistics (2023). Formula: floor area (m²) × heating demand (kWh/m²/year) × emission factor ÷ household size.',
        },
        {
          title:'⚡ Electricity',
          text:'Grid emission factor: 0.185 kg CO₂eq/kWh (EEA lifecycle methodology). This is higher than the direct combustion figure (Ember/Nowtricity: 0.110 kg/kWh) because it includes the nuclear fuel cycle, upstream gas methane leakage, grid transmission losses, and wind/solar infrastructure. All users use the same factor regardless of green contract type — unbundled Guarantees of Origin have negligible additionality (Brander et al. 2018; GHG Protocol Scope 2 Guidance). Solar panel lifecycle: 40 kg CO₂eq/kWp manufacturing, 25-year lifetime, 900 kWh/kWp/year yield (Belgium average). Electricity default values: Fluvius/VREG reference consumption by household size, adjusted by home size using an engineering multiplier (studio ×0.85, small ×0.90, medium ×1.0, large ×1.15, very large ×1.30).',
        },
        {
          title:'🍽️ Food & Diet',
          text:'Emission factors per kg food: Agribalyse 3.2 (ADEME) for most items; Poore & Nemecek 2018 (Science) for processed products (cheese, butter, eggs, bread, pasta, tofu, lentils) not yet available in Agribalyse agricultural file. Diet profile defaults: Scarborough et al. 2014 (Climatic Change) — Oxford EPIC cohort dietary patterns, adapted for Belgian food habits. Calorie modifiers: light 0.80×, average 1.00×, heavy 1.25× applied uniformly across all food groups. Food waste multipliers: low 1.10×, average 1.15×, high 1.25× (ADEME household waste methodology).',
        },
        {
          title:'🚗 Transport (non-flight)',
          text:'Emission factors per km: ADEME Base Empreinte well-to-wheel figures (includes upstream fuel production). Public transport: SNCB sustainability report (train), STIB environmental report (metro/tram), EEA/EMEP-EEA Guidebook 2023 (bus). Commuting calculation: one-way distance × 2 (return) × working days per year. Working days = (days/week × 52) − holiday days. Default holidays: 30 days (Belgian average public holidays + standard leave). Other trips: one-way distance × 2 × trips/week × 52.',
        },
        {
          title:'✈️ Flights',
          text:'Emission factors: DEFRA GHG Conversion Factors 2024, economy class, including Radiative Forcing Index (RFI ×2.0) accounting for contrails, NOx and water vapour effects at altitude. Short-haul (≤ 3,700 km): 0.153 kg CO₂eq/km/pax. Long-haul (> 3,700 km): 0.193 kg CO₂eq/km/pax. Routing detour correction: ×1.09 (DEFRA standard). Business class multiplier: ×1.5 (short-haul) / ×2.5 (long-haul). First class: ×4.0. Default distances: short-haul 1,200 km, medium-haul 3,000 km, long-haul 8,000 km (one-way great-circle).',
        },
        {
          title:'🛍️ Shopping',
          text:'Item-based (advanced): total product lifecycle CO₂eq allocated over product lifetime. Source: ADEME Base Empreinte for most items; supplemented by manufacturer LCAs (Apple, Dell, Samsung) and academic studies (MIT, GoClimate). Shared items (TV, appliances, furniture) divided by household size. Three lifetime tiers (short/average/long) from ADEME Base Empreinte categories. Spend-based (simple): spend-based emission factors (kg CO₂eq/€) from UK BEIS GHG Conversion Factors, applied to ADEME spend categories.',
        },
        {
          title:'♻️ Waste',
          text:'Two tiers based on ADEME lifecycle assessment methodology and IPCC waste treatment emission factors, calibrated to Belgian waste composition. Belgium uses incineration with energy recovery (not landfill) for residual waste — this reduces methane emissions compared to landfill-heavy systems. Careful recycler (~100 kg CO₂eq/year): residual waste < 50 kg/person/year, remainder sorted. Poor recycler (~320 kg CO₂eq/year): most waste in residual bin, higher incineration load. Source: ADEME/Fost Plus Belgium statistics.',
        },
        {
          title:'🐾 Pets',
          text:'Annual CO₂eq per pet type primarily from food (80–90% of footprint). Source: Okin GS (2017), "Dogs and cats as environmental pollutants" (PLOS ONE); Cerdá et al. (2021) for horses. Values represent mid-range estimates — actual footprint varies significantly with food brand, wet vs. dry food, and meat content.',
        },
      ].map(s=>(
        <div key={s.title} style={{marginBottom:14,paddingBottom:14,borderBottom:'1px solid #f3f4f6'}}>
          <div style={{fontWeight:700,fontSize:13,color:'#1a1a1a',marginBottom:5}}>{s.title}</div>
          <p style={{fontSize:13,lineHeight:1.75,color:'#374151',margin:0}}>{s.text}</p>
        </div>
      ))}
    </div>
  );
              ...
              <>
                ...
                {Object.entries(s.foodGrams).map(([k,g])=>(
                  <div key={k} style={{display:'flex',alignItems:'center',gap:12,marginBottom:10}}>
                    ...
                    <div style={{width:60,fontSize:13,color:'#4b5563',textAlign:'right'}}>{g} g/day</div>
                  </div>
                ))}
              </>
            )}
              ...
              {s.commuteMode==='single'
                ...
                : (
                  <div style={{background:'#f9fafb',borderRadius:8,padding:'14px 16px'}}>
                    <Caption>Adjust the share of each mode. Sliders auto-balance to 100%.</Caption>
                    {s.commuteModes.map((cm,idx)=>{
                      ...
                      return (
                        <div key={cm.mode} style={{display:'flex',alignItems:'center',gap:10,marginBottom:9}}>
                          <div style={{width:120,fontSize:13,color:'#374151',flexShrink:0}}>{opt.emoji} {opt.label}</div>
                          <input type="range" min={0} max={100} step={5} value={cm.share}
                            onChange={e=>updateCommuteShare(idx,Number(e.target.value))}
                            style={{flex:1,accentColor:ACCENT}} />
                          <div style={{width:32,fontSize:13,color:'#4b5563',textAlign:'right'}}>{cm.share}%</div>
                        </div>
                      );
                    })}
                    <div style={{fontSize:13,fontWeight:600,marginTop:4,color:s.commuteModes.reduce((t,m)=>t+m.share,0)===100?ACCENT:'#dc2626'}}>
