...
function DataSourceRow({ source, url }: { source: string; url: string | null }) {
  ...
  // If only one URL is given, apply it to all sources
  const getUrl  = (i: number) => urls[i] ?? (urls.length === 1 ? urls[0] : null);
  return (
    <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: 8 }}>
      {sources.map((s, i) => (
        <li key={i}>
          {getUrl(i)
            ? <a href={getUrl(i)!} target="_blank" rel="noopener noreferrer" className="detail-link">{s} ↗</a>
...
// Inland passenger modal split Belgium 2021 (Eurostat tran_hv_psmod, p-km)
const MODAL_SPLIT_BE = [
  { phase: 'Passenger car', value: 85,  color: '#94a3b8' },
  { phase: 'Bus & coach',   value: 8,   color: '#f59e0b' },
  { phase: 'Rail',          value: 7,   color: TOPIC_COLOR },
];
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
          {[
            ...
            <div key={i} style={{ borderRadius: 8, overflow: 'hidden', border: `1px solid ${col.color}33` }}>
              <div style={{ background: col.color, padding: '8px 14px' }}>
                <div style={{ fontWeight: 700, fontSize: '0.82rem', color: '#fff' }}>{col.label}</div>
              </div>
    ...
    <div className="detail-technical-card">
      ...
      <div style={{ borderTop: '1px solid #e5e7eb', paddingTop: 14, marginBottom: 16 }}>
        ...
        <p style={{ fontSize: '0.82rem', lineHeight: 1.7, color: '#374151', marginBottom: 10 }}>
          ...
          petrol car vs a BEV charged on the EU electricity mix.
        </p>
              ...
              <div style={{ width: '100%', height: 240 }}>
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart margin={{ top: 8, right: 0, bottom: 8, left: 0 }}>
                    <Pie data={panel.data} cx="50%" cy="40%" outerRadius={68} dataKey="value" nameKey="phase" labelLine={false}>
        ...
        <p style={{ fontSize: '0.72rem', color: '#9ca3af', marginTop: 8 }}>
          ...
          BEV figures assume average EU electricity; petrol at 7.5 L/100 km.
        </p>
      </div>
...
function PublicTransportTechnicalCard() {
  return (
    <div className="detail-technical-card">
      ...
      <div style={{ borderTop: '1px solid #e5e7eb', paddingTop: 14 }}>
        <strong style={{ display: 'block', marginBottom: 4 }}>Inland passenger modal split — Belgium 2021</strong>
        ...
        <div style={{ width: '100%', height: 230 }}>
          <ResponsiveContainer width="100%" height="100%">
            <PieChart margin={{ top: 8, right: 0, bottom: 8, left: 0 }}>
              <Pie data={MODAL_SPLIT_BE} cx="50%" cy="42%" outerRadius={78} dataKey="value" nameKey="phase" labelLine={false}>
                {MODAL_SPLIT_BE.map((e, j) => <Cell key={j} fill={e.color} stroke="white" strokeWidth={2} />)}
              </Pie>
              <Tooltip contentStyle={{ background: '#fff', border: '1px solid #e5e3da', borderRadius: 8, fontSize: 11 }}
                formatter={(v: any, n: any) => [`${v}%`, n]} />
              <Legend iconType="circle" iconSize={9} formatter={v => <span style={{ fontSize: '0.78rem', color: '#4b5563' }}>{v}</span>} />
            </PieChart>
          </ResponsiveContainer>
        </div>
        <p style={{ fontSize: '0.68rem', color: '#9ca3af', marginBottom: 12 }}>
          Source: Eurostat tran_hv_psmod; EC DG MOVE European Transport in Figures 2024. EU avg car share: ~71%.
        </p>

        <div style={{ background: '#eff6ff', borderRadius: 8, padding: '10px 14px', border: '1px solid #bfdbfe', marginBottom: 8 }}>
          <p style={{ fontSize: '0.82rem', color: '#1e3a8a', margin: 0, lineHeight: 1.6 }}>
            <strong>🚋 Tram &amp; metro not included:</strong> Brussels (STIB/MIVB), Antwerp, Ghent and Liège
            operate trams and/or metro lines but their passenger-kilometres are not harmonised into Eurostat&#39;s
            inland modal split methodology. Including them would raise the public transport share by an estimated 1–2 pp.
          </p>
        </div>

        <div style={{ background: '#fafafa', borderRadius: 8, padding: '10px 14px', border: '1px solid #e5e7eb' }}>
          <p style={{ fontSize: '0.82rem', color: '#374151', margin: 0, lineHeight: 1.6 }}>
            <strong>Why no binding target?</strong> Belgium has no national legally binding modal shift target
            for passenger transport. The EU&#39;s Sustainable &amp; Smart Mobility Strategy (2020) calls
            for doubling rail passenger traffic by 2030 (vs 2015), but this is a political
            commitment, not an enforceable obligation. Belgium&#39;s 85% car share has been
            effectively unchanged since 2010.
          </p>
        </div>
      </div>
    </div>
...
// Road freight modal share — EU country comparison 2022 (Eurostat tran_hv_frmod, %)
const EU_FREIGHT_ROAD = [
  { country: 'Lithuania',   pct: 92, color: '#dc2626' },
  { country: 'Estonia',     pct: 91, color: '#dc2626' },
  { country: 'Latvia',      pct: 89, color: '#dc2626' },
  { country: 'Ireland',     pct: 95, color: '#dc2626' },
  { country: 'Cyprus',      pct: 100,color: '#dc2626' },
  { country: 'Malta',       pct: 100,color: '#dc2626' },
  { country: 'Greece',      pct: 98, color: '#dc2626' },
  { country: 'Spain',       pct: 85, color: '#f97316' },
  { country: 'Portugal',    pct: 83, color: '#f97316' },
  { country: 'Bulgaria',    pct: 81, color: '#f97316' },
  { country: 'Romania',     pct: 78, color: '#f97316' },
  { country: 'Hungary',     pct: 77, color: '#f97316' },
  { country: 'Poland',      pct: 76, color: '#f97316' },
  { country: 'Belgium',     pct: 75, color: '#ec4899' },
  { country: 'EU target',   pct: 64, color: '#16a34a' },
  { country: 'France',      pct: 72, color: '#fbbf24' },
  { country: 'Italy',       pct: 70, color: '#fbbf24' },
  { country: 'Slovenia',    pct: 67, color: '#fbbf24' },
  { country: 'Czech Rep.',  pct: 63, color: '#22c55e' },
  { country: 'Slovakia',    pct: 60, color: '#22c55e' },
  { country: 'Germany',     pct: 58, color: '#22c55e' },
  { country: 'Austria',     pct: 56, color: '#22c55e' },
  { country: 'Netherlands', pct: 51, color: '#22c55e' },
  { country: 'Switzerland', pct: 38, color: '#22c55e' },
...
function FreightTechnicalCard() {
  return (
    <div className="detail-technical-card">
      ...
      <div style={{ marginBottom: 16 }}>
        <strong style={{ display: 'block', marginBottom: 4 }}>Road freight share — EU country comparison (2022, %)</strong>
        <p style={{ fontSize: '0.75rem', color: '#6b7280', margin: '0 0 8px' }}>
          Share of inland freight carried by road (tonne-km). Sorted highest to lowest.
          Belgium (pink) and EU 2030 target (green) highlighted. Source: Eurostat tran_hv_frmod.
        </p>
        <div style={{ width: '100%', height: EU_FREIGHT_ROAD.length * 24 + 40 }}>
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={EU_FREIGHT_ROAD} layout="vertical" margin={{ top: 4, right: 48, left: 90, bottom: 4 }} barSize={14}>
              <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" horizontal={false} />
              <XAxis type="number" domain={[0, 105]} tick={{ fontSize: 10, fill: '#9ca3af' }} tickLine={false} axisLine={false}
                tickFormatter={v => `${v}%`} />
              <YAxis type="category" dataKey="country" tick={(props: any) => {
                const isBE = props.payload.value === 'Belgium';
                const isTarget = props.payload.value === 'EU target';
                return (
                  <text x={props.x} y={props.y} dy={4} textAnchor="end" fill={isBE ? '#ec4899' : isTarget ? '#16a34a' : '#374151'}
                    fontSize={isBE || isTarget ? 11 : 10} fontWeight={isBE || isTarget ? 700 : 400}>
                    {props.payload.value}
                  </text>
                );
              }} tickLine={false} axisLine={false} width={88} interval={0} />
              <Tooltip contentStyle={{ background: '#fff', border: '1px solid #e5e3da', borderRadius: 8, fontSize: 11 }}
                formatter={(v: any) => [`${v}%`, 'Road share']} />
              <Bar dataKey="pct" radius={[0, 3, 3, 0]}>
                {EU_FREIGHT_ROAD.map((d, i) => <Cell key={i} fill={d.color} />)}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        </div>
...
export default function MobilityDetailPage() {
  ...
  if (!indicatorName) return (
    <div style={{ padding: 48, textAlign: 'center' }}>
      ...
      <Link href="/indicators?topic=mobility_transport" style={{ color: TOPIC_COLOR, fontWeight: 600 }}>← Back to overview</Link>
    </div>
  );
    ...
    <div className="detail-page">
      <div className="detail-header" style={{ background: '#1a1a1a' }}>
        <div className="detail-header-inner">
          <button onClick={() => router.back()} className="back-link" style={{ background: 'none', border: 'none', cursor: 'pointer' }}>← Back to overview</button>
