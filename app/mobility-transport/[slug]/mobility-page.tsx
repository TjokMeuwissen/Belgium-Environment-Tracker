...
const SECTION_DEFS: Record<string, { id: string; label: string }[]> = {
  'zero-emission-new-car-registrations': [
    ...
    { id: 'main-chart',     label: 'EU electricity intensity' },
              ...
              <div style={{ width: '100%', height: 200 }}>
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart margin={{ top: 8, right: 0, bottom: 0, left: 0 }}>
                    <Pie data={panel.data} cx="50%" cy="44%" outerRadius={68} dataKey="value" labelLine={false}>
          ...
          {[
            ...
            <div key={i} style={{ background: '#fafafa', borderRadius: 8, padding: '12px 14px', border: `1px solid #f0f0f0`, borderLeft: `4px solid ${item.color}` }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 4, marginBottom: 8 }}>
                {item.hubs.map((h, j) => (
                  <div key={j} style={{ fontSize: '0.78rem', color: '#374151', paddingLeft: 6 }}>• {h}</div>
                ))}
              </div>
...
// ── Chart blocks ──────────────────────────────────────────────────────────────
function ElectricityIntensityChart() {
  return (
    <div className="detail-chart-block">
      <div className="detail-chart-title">Electricity carbon intensity — selected EU countries (2023, gCO₂/kWh)</div>
      <div style={{ width: '100%', height: 260 }}>
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={ELEC_INTENSITY} layout="vertical" margin={{ top: 4, right: 50, left: 10, bottom: 4 }}>
            <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" horizontal={false} />
            <XAxis type="number" tick={{ fontSize: 11, fill: '#9ca3af' }} tickLine={false} axisLine={false}
              domain={[0, 700]} tickFormatter={v => `${v}`} />
            <YAxis type="category" dataKey="country" tick={{ fontSize: 11, fill: '#374151' }} tickLine={false} axisLine={false} width={95} interval={0} />
            ...
            <Bar dataKey="gco2" radius={[0, 3, 3, 0]}>
              {ELEC_INTENSITY.map((d, i) => <Cell key={i} fill={d.color} />)}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
      </div>
      <p className="detail-chart-source">
        Source: Ember European Electricity Review 2024; EEA ENER 038 indicator. 2023 data.
        Belgium (138 gCO₂/kWh) is well below the EU average thanks to its nuclear fleet; Poland remains the highest due to 61% coal share.
      </p>
    </div>
  );
}
...
export default function MobilityDetailPage() {
  ...
  const chartNode: Record<string, React.ReactNode> = {
    'zero-emission-new-car-registrations': <ElectricityIntensityChart />,
