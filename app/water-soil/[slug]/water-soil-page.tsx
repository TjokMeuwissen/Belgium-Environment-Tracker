...
function ResponsibilityCard({ text }: { text: string }) {
  // Extract primary level note ("Regional (primary)." etc.) and show as banner
  ...
  const primaryColors: Record<string, { bg: string; border: string; text: string }> = {
  ...
  const pc = primaryLevel ? (primaryColors[primaryLevel] ?? primaryColors.Regional) : null;
    ...
    <div className="detail-responsibility-card">
      ...
      {pc && primaryLevel && (
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: pc.bg, border: `1px solid ${pc.border}`, borderRadius: 8, padding: '6px 14px', marginBottom: 12 }}>
          <span style={{ fontSize: '0.75rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em', color: pc.text }}>
            Primary responsibility
          </span>
          <span style={{ background: pc.border, color: 'white', borderRadius: 4, padding: '2px 8px', fontSize: '0.78rem', fontWeight: 700 }}>
            {primaryLevel}
          </span>
        </div>
      )}
