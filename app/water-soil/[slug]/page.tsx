    ...
    <div className="detail-technical-card">
      ...
      <div style={{ borderTop: '1px solid #e5e7eb', paddingTop: 16, marginBottom: 18 }}>
        ...
        <ResponsiveContainer width="100%" height={220}>
          <BarChart data={barData} layout="vertical" margin={{ top: 4, right: 40, left: 80, bottom: 4 }}>
            <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" horizontal={false} />
            <XAxis type="number" tick={{ fontSize: 11, fill: '#9ca3af' }} tickLine={false} axisLine={false} tickFormatter={v => `${v}%`} domain={[0, 30]} />
            <YAxis type="category" dataKey="country" tick={{ fontSize: 11, fill: '#374151' }} tickLine={false} axisLine={false} width={75} />
      ...
      <div style={{ borderTop: '1px solid #e5e7eb', paddingTop: 16 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          {[
            ['💧 Disrupts the water cycle', 'Sealed surfaces prevent rainwater from infiltrating into the soil. Instead of recharging groundwater aquifers, water runs off rapidly into drainage systems, increasing flood peaks downstream and reducing groundwater recharge — directly worsening Belgium\'s water stress.'],
            ['🌡️ Urban heat island effect', 'Impervious surfaces absorb and retain solar heat, raising urban temperatures by up to 5°C relative to surrounding countryside. This increases energy demand for cooling and heat-related health risks — a growing concern for Belgian cities in heatwaves.'],
            ['🦋 Habitat fragmentation & biodiversity loss', 'Sealed land eliminates soil biodiversity (earthworms, fungi, bacteria) essential for carbon storage and nutrient cycling. It fragments wildlife corridors and destroys habitats for ground-nesting insects and plants.'],
            ['🌊 Increased flood risk', 'Rapid runoff from impervious surfaces overwhelms urban drainage systems, contributing to flash flooding. Belgium\'s 2021 floods (Liège, Namur) were worsened by high soil sealing in valley catchments.'],
            ['🌱 Loss of agricultural land', 'Once sealed, land is almost impossible to restore to productive agricultural use. Belgium loses approximately 6 km² of agricultural land per year to urbanisation — a permanent and irreversible resource loss.'],
          ].map(([title, text], i) => (
            <div key={i} style={{ display: 'flex', gap: 10, padding: '8px 12px', background: '#fafafa', borderRadius: 6, border: '1px solid #f0f0f0' }}>
              <span style={{ flexShrink: 0, fontWeight: 700, fontSize: '0.82rem', color: '#1a1a1a', minWidth: 220 }}>{title}</span>
              <p style={{ fontSize: '0.82rem', color: '#4b5563', margin: 0, lineHeight: 1.5 }}>{text}</p>
            </div>
          ))}
        </div>
