...
function ConsequencesCard({ text }: { text: string }) {
  return (
    <div className="detail-consequences-card">
      ...
      <p style={{ fontSize: '0.88rem', lineHeight: 1.75, color: '#374151', margin: 0 }}>{text}</p>
    </div>
  );
}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          {[
            ...
            },
            {
              ...
              output: 'Biogas → replaces natural gas · Digestate → replaces synthetic fertiliser',
            },
          ].map((item, i) => (
            <div key={i} style={{ background: item.bg, borderRadius: 8, padding: '14px 16px', borderLeft: `4px solid ${item.color}` }}>
              ...
              <div style={{ fontSize: '0.75rem', fontWeight: 600, color: item.color }}>📦 {item.output}</div>
            </div>
          ))}
        </div>
...
function CMURTechnicalCard({ cmurByMaterial }: { cmurByMaterial: any[] }) {
  ...
  return (
    <div className="detail-technical-card">
      ...
      <div style={{ borderTop: '1px solid #e5e7eb', paddingTop: 18 }}>
        <strong style={{ display: 'block', marginBottom: 8 }}>Composition of Belgium&#39;s recycled material stream</strong>
        <ResponsiveContainer width="100%" height={240}>
          <PieChart>
            <Pie
...
export default function CircularityDetailPage() {
  ...
  if (slug === 'municipal-solid-waste-recycling-rate') {
    ...
    chartNode = (
      <div className="detail-chart-block">
        <div className="detail-chart-title">Municipal Waste Generation per Capita — Belgium 2004–2023 (kg/cap)</div>
        <ResponsiveContainer width="100%" height={280}>
          <LineChart data={historicalWaste} margin={{ top: 8, right: 24, left: 0, bottom: 0 }}>
            <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" />
            <XAxis dataKey="year" tick={{ fontSize: 11, fill: '#9ca3af' }} tickLine={false} interval={3} />
            <YAxis tick={{ fontSize: 11, fill: '#9ca3af' }} tickLine={false} axisLine={false} width={40} domain={[400, 800]} />
            <Tooltip contentStyle={{ background: '#fff', border: '1px solid #e5e3da', borderRadius: 8, fontSize: 12 }}
              formatter={(v: any) => [`${v} kg/cap`, 'Waste generation']} />
            <ReferenceLine y={513} stroke={TOPIC_COLOR} strokeDasharray="6 4" strokeWidth={1.8}
              label={{ value: '🎯 2030 target: 513 kg/cap', position: 'insideTopRight', fontSize: 11, fill: TOPIC_COLOR, fontWeight: 600 }} />
            <Line type="monotone" dataKey="value" stroke={TOPIC_COLOR} strokeWidth={2.5} dot={false} activeDot={{ r: 5, fill: TOPIC_COLOR }} />
          </LineChart>
        </ResponsiveContainer>
        <p className="detail-chart-source">Source: Statbel / Eurostat (env_wasmun). ⚠️ Series break at 2020 — new EU definition.</p>
